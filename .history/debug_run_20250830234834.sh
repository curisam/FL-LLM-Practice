...
if command -v torchrun >/dev/null 2>&1; then
  RUN_CMD=(torchrun --nproc_per_node="$NPROC" --master_port="$MASTER_PORT" "$MAINPY" --cfg "$CFG" llm.accelerator.use True)
else
  RUN_CMD=(python -m torch.distributed.run --nproc_per_node="$NPROC" --master_port="$MASTER_PORT" "$MAINPY" --cfg "$CFG" llm.accelerator.use True)
fi
