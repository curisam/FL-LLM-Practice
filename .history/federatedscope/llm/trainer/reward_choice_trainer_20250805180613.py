...
class RewardChoiceTrainer(LLMTrainer):
    ...
    # --- train 진입 시 스케줄러 step 및 로더 리빌드 ---
    @use_diff
    def train(self, target_data_split_name="train", hooks_set=None):
        # ==================== 수정된 부분 시작 ====================
        # 매 라운드(train 함수 호출 시) 스케줄러의 step()을 호출
        if self.scheduler is not None:
            self.scheduler.step()  # 인자 없이 호출하여 내부 카운터 증가
            current_lr = self.scheduler.get_last_lr()[0]
            # 현재 라운드(스케줄러의 epoch 카운터 기준)와 LR 로깅
            logger.info(f"[LRScheduler] Stepped for round {self.scheduler.last_epoch}. "
                        f"New LR: {current_lr:.2e}")
        # ==================== 수정된 부분 끝 ======================
    ...
    def _hook_on_fit_start_init(self, ctx):
        ...
        # ==================== 수정된 부분 시작 ====================
        # 스케줄러가 아직 생성되지 않았을 경우에만 생성
        if self.scheduler is None:
            sched_cfg = getattr(self.cfg.train, "scheduler", None)
            if sched_cfg and getattr(sched_cfg, "type", "") == "MultiStepLR":
                opt = getattr(ctx, "optimizer", None)
                if opt is None:
                    # fallback to self.optimizer if ctx.optimizer is not available
                    opt = getattr(self, "optimizer", None)
                ...
                if opt:
                    ...
                    logger.info(f"MultiStepLR scheduler initialized with milestones={sched_cfg.milestones} and gamma={sched_cfg.gamma}")
                else:
                    logger.warning("[LRScheduler] Optimizer not found. LR Scheduler will not be created.")
        # ==================== 수정된 부분 끝 ======================


    # --- (이하 _hook_on_batch_forward, _hook_on_batch_end 등 나머지 함수들은 기존과 동일) ---
    def _hook_on_batch_forward(self, ctx):
        input_ids = ctx.data_batch['input_ids'].to(ctx.device)
        labels = ctx.data_batch['labels'].to(ctx.device)
        attention_mask = ctx.data_batch['attention_mask'].to(ctx.device)
