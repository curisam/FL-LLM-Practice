...
from transformers import AdamW  # transformers.AdamW: 경고는 무시 가능
...
class LLMTrainer(GeneralTorchTrainer):
    ...
    def _run_batch(self, hooks_set, run_step=-1):
        ...
        if self.ctx.cur_mode in [MODE.TRAIN, MODE.FINETUNE]:
            ...
            if hasattr(self, 'accelerator') and self.accelerator is not None:
                ...
                logger.info(f"[{split}|accelerate] planned_micro={planned_micro}, "
                            f"done_micro={done_micro}, this_run_micro={run_step}, "
                            f"grad_accum={grad_accum_step}")
    ...
    def _hook_on_fit_start_init(self, ctx):
        ...
        # 라운드 시작 시 진행 스텝 카운터 리셋 (중복 호출 방지용)
