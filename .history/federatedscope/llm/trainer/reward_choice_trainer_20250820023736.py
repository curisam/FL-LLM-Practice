...
class RewardChoiceTrainer(LLMTrainer): #LLM이 뱉는 logits[B, T, V](V=전체 vocab)에 대해 “다음 토큰이 A냐 B냐만” 보도록 축소해서 **CrossEntropyLoss(B×(T-1) vs 2)**로 학습/평가한다. 
    ...
    def _hook_on_fit_end(self, ctx): 
        """
        라운드 종료 시점에 각 split(train/val/test)별로:
        1. 전역 샘플 수와 손실 합계 reduce

        2. 평균 손실, 정확도 계산

        3. 메인 프로세스에서만 metric dict 업데이트 및 로그 출력

        즉, 배치별 누적 통계를 최종 평가 지표로 변환하는 최종 단계입니다.
        ...
        #4) Local 에서 정확도 계산 준비. 샘플/정답 집계 텐서화
        t_seen = torch.tensor(
        ...
        if is_main:
            ...
            })#update의 경우 기존 key가 존재한다면 덮어쓰기 함. 없으면 새 key가 추가.
