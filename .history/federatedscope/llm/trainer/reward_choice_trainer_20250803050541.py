...
class RewardChoiceTrainer(LLMTrainer): #LLMTrainer를 상속 받아, “여러 선택지 중 정답을 고르는” 형태의 LLM 파인튜닝 과제를 지원하도록 만든 커스텀 트레이너입니다.
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        # -------- 샘플 단위(correct/total) 계산 --------
        # new_logits: [B, S-1, C], new_labels: [B, S-1]
        ...
        # 각 샘플에서 "처음 등장하는 유효 위치"를 하나 잡아서 판정
        # (없으면 0이지만 has_valid로 필터링)
        ...
        if sel_b.numel() > 0:
            ...
            pred_1st   = torch.argmax(logits_1st, dim=-1)        # [Bv]
        ...
        # 이후 필요하면 토큰단위 정보도 유지
        # (y_true/y_prob가 필요 없으면 이 블록은 생략해도 됨)
        ...
        # 참고: 아래 값은 쓰지 않습니다. (우리는 sample_count로 누적)
    ...
    def _hook_on_batch_end(self, ctx):
        ...
        # 평가 모드에서 원시 예측을 쌓고 싶으면 여기는 그대로 두세요.
        if ctx.cur_mode in [MODE.TEST, MODE.VAL]:
            if not hasattr(ctx, 'ys_true'): ctx.ys_true = []
            if not hasattr(ctx, 'ys_pred'): ctx.ys_pred = []
            pred = torch.argmax(ctx.y_prob, dim=-1)
    ...
    def _hook_on_fit_end(self, ctx):
        """
        split 종료 시, 모든 프로세스의 통계를 합산하여 rank0만 eval_metrics를 채웁니다.
        (train/val/test 공통)  -> Client는 이 값만 파일/서버에 기록/전송.
        """
        split = getattr(ctx, "cur_split", None)
        if split not in ("train", "val", "test"):
            return
        ...
        if is_main:
            ...
            ctx.eval_metrics[f"{split}_total"]     = n                          # ex) train_total = 480
            ctx.eval_metrics[f"{split}_loss"]      = loss_total                 # 합계
            ctx.eval_metrics[f"{split}_avg_loss"]  = loss_total / max(n, 1)     # 평균
            ctx.eval_metrics[f"{split}_acc"]       = correct / max(n, 1)        # 정확도
        else:
            # 비-main 프로세스는 외부에 노출(파일/서버)되지 않도록 비워둠
            ctx.eval_metrics = {}
