...
class RewardChoiceTrainer(LLMTrainer):
        ...
        if hasattr(client_data, data_key) and getattr(client_data, data_key) is not None:
            ...
            if world_size > 1:
                ...
                if split_name == 'train':
                    ...
                    sampler = EvalShardSampler(len(dataset), rank, world_size)
                    loader = DataLoader(
                        dataset=dataset,
                        batch_size=bs,
                        sampler=sampler,
                ...
                loader = DataLoader(
                    ...
                    num_workers=num_workers,
                    pin_memory=pin_memory,
                    drop_last=(split_name=='train' and drop_last),
                    collate_fn=self.data_collator,
                )
            ...
            logger.info(
                f"Dataloader for '{split_name}' reset: "
                f"local_count={len(loader.dataset)}, total={len(base_loader.dataset)}"
            )
    ...
    def train(self, target_data_split_name='train', hooks_set=None):
        # FL 라운드 기준 스케줄러 스텝
        if hasattr(self, 'scheduler'):
            round_idx = getattr(self.ctx, 'cur_state', 0)
            self.scheduler.step(round_idx)
            lr = self.scheduler.get_last_lr()[0]
            logger.info(f"[LRScheduler] Round {round_idx} → lr={lr:.2e}")
    ...
    # (이하 생략: evaluate, hook_on_fit_start_init, hook_on_batch_forward/end, _hook_on_fit_end 등 동일)
