...
#### 2. Mixed precision (예: bf16)과 gradient accumulation → Accelerate-> 이게 실제로 적용됨.
    ...
    def train(self, target_data_split_name="train", hooks_set=None, round_num=-1):
        ...
        try:
            ...
            for k in ('optimizer', 'scheduler', 'grad_scaler', 'scaler'):
        ...
        for update_i in range(run_step):
            ...
            if hasattr(self, 'accelerator'): #accelerator 모드에서 실행. 
                ...
                if self._ct_ft and self._mid_eval_every > 0 and self.ctx.cur_mode in [MODE.TRAIN, MODE.FINETUNE]:
                    if self.accelerator.sync_gradients:  # ← 실제 optimizer.step() 시점
    ...
    def _mid_eval_once(self):
        ...
        # ---------- (3) VAL/TEST mid-eval ----------
        splits = list(set(getattr(self.cfg.eval, "split", [])))  # 예: ['val','test']
        for sp in splits:
