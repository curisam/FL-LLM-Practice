...
class RewardChoiceTrainer(LLMTrainer):
    ...
    def _hook_on_fit_start_init(self, ctx):
        ...
        # ==================== Stateless LR 제어 로직 (최종 수정) ====================

        # 1. 현재 FL 라운드 번호를 올바른 변수인 `ctx.round_num`에서 가져옵니다.
        #    `getattr`를 사용하여 변수가 없을 경우 안전하게 0으로 처리합니다.
        current_round = int(getattr(ctx, "round_num", 0))
        ...
        if scheduler_cfg:
            ...
            for milestone in sorted(milestones):
                # 라운드 인덱스가 milestone 값보다 크거나 같으면 decay
    ...
    def _hook_on_batch_end(self, ctx):
        ...
        if opt is not None:
            current_lr = opt.param_groups[0]["lr"]
            round_idx = getattr(ctx, "round_num", getattr(ctx, "cur_state", "?")) # 올바른 변수 우선 사용
            batch_idx = getattr(ctx, "batch_idx", "?")
