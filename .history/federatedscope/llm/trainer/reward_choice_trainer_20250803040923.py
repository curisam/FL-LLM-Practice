...
class RewardChoiceTrainer(LLMTrainer): #LLMTrainer를 상속 받아, “여러 선택지 중 정답을 고르는” 형태의 LLM 파인튜닝 과제를 지원하도록 만든 커스텀 트레이너입니다.
    ...
    def _hook_on_batch_end(self, ctx):
        ...
        # === 1) gradient accumulation 보정 계수 ===
        accum = int(getattr(self.cfg.llm, "grad_accum_step", 1))  # 예: 2
        accum = max(1, accum)

        # === 2) 이 트레이너의 "한 배치가 의미하는 유효 샘플 수" ===
        bs = int(ctx.batch_size)                                  # 시퀀스 개수 (예: 2)
        pair_factor = max(1, len(getattr(self, 'choices', [])))   # 선택지 수 (예: 2)
        # micro-batch 기준 증가량을 optimizer-step 기준으로 정규화
        incr = (bs * pair_factor) / float(accum)                  # 예: 4 / 2 = 2.0

        # === 3) split별 누적 키 준비(안전 초기화) ===
        loss_total_key  = f'loss_total_{split}'
        correct_key     = f'correct_{split}'
        num_samples_key = f'num_samples_{split}'
        if not hasattr(ctx, loss_total_key):   setattr(ctx, loss_total_key, 0.0)
        if not hasattr(ctx, correct_key):      setattr(ctx, correct_key, 0.0)  # float로 둡니다
        if not hasattr(ctx, num_samples_key):  setattr(ctx, num_samples_key, 0.0)

        # === 4) 손실/정답/샘플 누적 (optimizer-step 기준 정규화) ===
        # loss_batch는 보통 평균 loss이므로, "유효 샘플 수"를 곱해줍니다.
        setattr(ctx, loss_total_key,  getattr(ctx, loss_total_key)  + float(ctx.loss_batch.item()) * incr)

        pred    = torch.argmax(ctx.y_prob, dim=-1)
        correct = float((pred == ctx.y_true).sum().item()) / float(accum)      # 정답수도 정규화
        setattr(ctx, correct_key,     getattr(ctx, correct_key)     + correct)

        setattr(ctx, num_samples_key, getattr(ctx, num_samples_key) + incr)

        # === 5) 평가 모드일 때만 원시 예측 저장(옵션) ===
        if ctx.cur_mode in [MODE.TEST, MODE.VAL]:
            if not hasattr(ctx, 'ys_true'): ctx.ys_true = []
            if not hasattr(ctx, 'ys_pred'): ctx.ys_pred = []
            ctx.ys_true.append(ctx.y_true)
    ...
    def _hook_on_fit_end(self, ctx):
        ...
        if split in ["train", "test", "val"]:
            n = float(getattr(ctx, f"num_samples_{split}", 0.0))
            loss_total = float(getattr(ctx, f"loss_total_{split}", 0.0))
            correct = float(getattr(ctx, f"correct_{split}", 0.0))

            # generic 키 (Client가 Results_raw 만들 때 참조)
            ctx.total = int(round(n))                            # ← 120(랭크당)로 귀결
            ctx.loss = loss_total
            ctx.avg_loss = loss_total / max(n, 1.0)
            ctx.acc = correct / max(n, 1.0)
