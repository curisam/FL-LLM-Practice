...
#### 2. Mixed precision (예: bf16)과 gradient accumulation → Accelerate-> 이게 실제로 적용됨.
    ...
    def _run_batch(self, hooks_set, run_step=-1): #한 epoch 내에서 batch 1개 도는 걸 여러번 반복. run_epoch에서는 run_step=-1 적용. 다른 곳에서는 run_step=1 적용.
        ...
        for update_i in range(run_step):
            ...
            if hasattr(self, 'accelerator'): #accelerator 모드에서 실행. 
                ...
                # ★ grad_accum_step번의 micro-batch가 끝나면 '업데이트 1회' 완료
                if self._ct_ft and self._mid_eval_every > 0 and self.ctx.cur_mode in [MODE.TRAIN, MODE.FINETUNE]:
                    boundary = ((update_i + 1) % max(1, grad_accum_step) == 0)
                    if boundary:
                        self._global_updates += 1
                        if (self._global_updates % self._mid_eval_every) == 0:
                            self._mid_eval_once()
    ...
    def _mid_eval_once(self):
        """CT-FT에서 every_n_train_steps마다: (1) train 스냅샷, (2) VAL/TEST mid-eval."""
        ...
        if is_main and train_snap:
            outdir = getattr(self.cfg.eval, "outdir", "runs")
            os.makedirs(outdir, exist_ok=True)
            path = os.path.join(outdir, "train_results.raw")   # ← 요청하신 파일
            cid  = getattr(getattr(self, "ctx", object()), "client_ID", 0)
            rnd  = getattr(getattr(self, "ctx", object()), "round", 0)
            adapter_idx = getattr(getattr(self, "ctx", object()), "current_adapter_idx", None)

            payload = {
                "client": int(cid),
                "round":  int(rnd),
                "step":   int(getattr(self, "_global_updates", 0)),  # 옵티마이저 업데이트 기준 step
                "phase":  "train_mid",
        ...
        # ---------- (2) VAL / TEST mid-eval ----------
        results = {}
        for sp in set(getattr(self.cfg.eval, "split", [])):   # 예: ['val','test']
            out = self.evaluate(target_data_split_name=sp)    # 기존 경로 사용(_run_routine+reduce)
            results[sp] = out

        if is_main:
            outdir = getattr(self.cfg.eval, "outdir", "runs/mid_eval")
            os.makedirs(outdir, exist_ok=True)
            cid = getattr(getattr(self, "ctx", object()), "client_ID", 0)
            rnd = getattr(getattr(self, "ctx", object()), "round", 0)
            path = os.path.join(outdir, f"client_{cid:03d}.jsonl")
    ...
    def _train_snapshot_metrics(self):
        """현재까지 누적된 train 통계를(분산이면 reduce로) 스냅샷."""
        split = "train"
