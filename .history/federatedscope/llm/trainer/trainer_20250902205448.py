...
#### 2. Mixed precision (예: bf16)과 gradient accumulation → Accelerate-> 이게 실제로 적용됨.
    ...
    def _hook_on_fit_end_free_space(self, ctx):
        ...
        # 🚩 추가: 언랩된 원본 모델을 CPU로 내려 VRAM 해제 (free_memory 호출 전에)
        try:
            base = self._unwrap(getattr(self.ctx, "model", None)) or self._unwrap(self.model)
            if base is not None:
                with torch.no_grad():
                ...
                logger.info("Model moved to CPU and CUDA cache cleared.")
        except Exception as e:
            logger.warning(f"CPU offload skipped: {e}")
