...
class RewardChoiceTrainer(LLMTrainer):
    def __init__(self, model, data, device, config, only_for_eval=False, monitor=None):
        ...
        # ── 여기에 스케줄러 초기화 코드 추가 ──
        sched_cfg = getattr(config.train, "scheduler", None)
        if sched_cfg and getattr(sched_cfg, "type", "") == "MultiStepLR":
            self.scheduler = MultiStepLR(
                self.optimizer,              # 부모 클래스에서 이미 생성된 optimizer
    ...
    def _hook_on_batch_end(self, ctx):
        ...
        # ── 여기에 LR 디버그 로그 추가 ──
        # optimizer 가 MultiStepLR 에 붙어 있으면 .param_groups[0]['lr'] 에 최신값이 담겨 있습니다.
        current_lr = self.optimizer.param_groups[0]['lr']
        logger.debug(f"[LR DEBUG] round={ctx.cur_state} split={ctx.cur_split} "
                     f"batch_idx={getattr(ctx, 'batch_idx', '?')} lr={current_lr:.2e}")
