...
#### 2. Mixed precision (예: bf16)과 gradient accumulation → Accelerate-> 이게 실제로 적용됨.
    ...
    def __init__(self,
        ...
        self._mid_eval_call_post = False  # ← 추가
    ...
    def _run_batch(self, hooks_set, run_step=-1): #한 epoch 내에서 batch 1개 도는 걸 여러번 반복. run_epoch에서는 run_step=-1 적용. 다른 곳에서는 run_step=1 적용.
        ...
        for update_i in range(run_step):
            ...
            if self.ctx.cur_mode in [MODE.TRAIN, MODE.FINETUNE] and self.ctx.cur_epoch_i == self.ctx.num_train_epoch - 1: # 학습/파인튜닝 모드 확인 및 마지막 에폭인지 확인
    ...
    def _hook_on_batch_forward(self, ctx): #ctx.data_batch → input_ids, attention_mask, labels로 명시 처리 + 모델 호출 방식 변경. 기존 toprch trainer는 CNN, MLP, BERT 용이라 LLM용은 아니라서 OVERRIDE 해야함.
        ...
        # ✅ 배치 크기/기본 loss 값은 무조건 먼저 세팅
        bs = int(labels.size(0))
        ctx.batch_size = CtxVar(bs, LIFECYCLE.BATCH)
        ...
        if ctx.cfg.llm.accelerator.use:
            ...
            if getattr(self.accelerator, "sync_gradients", True): 
                ...
                if self._ct_ft and self._mid_eval_every > 0 and (self._global_updates % self._mid_eval_every) == 0:
                    ...
                    # === ⬇️ 스냅샷 ===
                    _bs   = self.ctx.get('batch_size', None)
                    _loss = self.ctx.get('loss_batch', None)
                    _scnt = int(getattr(self.ctx, 'sample_count_batch', 0))
                    _scor = int(getattr(self.ctx, 'sample_correct_batch', 0))

                    if not getattr(self, "_mid_eval_running", False):
                        try:
                    ...
                    # === ⬇️ 복구(반드시 CtxVar로) ===
                    if _bs is not None:
                        self.ctx.batch_size = CtxVar(_bs, LIFECYCLE.BATCH)
                    if _loss is not None:
                        self.ctx.loss_batch = CtxVar(_loss, LIFECYCLE.BATCH)
                    self.ctx.sample_count_batch   = _scnt
                    self.ctx.sample_correct_accum = int(getattr(self.ctx, 'sample_correct_accum', 0))  # 누적은 유지
                    self.ctx.sample_correct_batch = _scor
            ...
            if (ctx.cur_batch_i + 1) % self.grad_accum_step == 0: #조건부 업데이트. cur_batch_i+1) % grad_accum_step == 0일 때만
                ...
                if self._ct_ft and self._mid_eval_every > 0 and (self._global_updates % self._mid_eval_every) == 0:
                    # === ⬇️ 스냅샷 ===
                    _bs   = ctx.get('batch_size', None)
                    _loss = ctx.get('loss_batch', None)
                    _scnt = int(getattr(ctx, 'sample_count_batch', 0))
                    _scor = int(getattr(ctx, 'sample_correct_batch', 0))
                    ...
                    # === ⬇️ 복구 ===
                    if _bs is not None:
                        ctx.batch_size = CtxVar(_bs, LIFECYCLE.BATCH)
                    if _loss is not None:
                        ctx.loss_batch = CtxVar(_loss, LIFECYCLE.BATCH)
                    ctx.sample_count_batch   = _scnt
                    ctx.sample_correct_accum = int(getattr(ctx, 'sample_correct_accum', 0))
                    ctx.sample_correct_batch = _scor
