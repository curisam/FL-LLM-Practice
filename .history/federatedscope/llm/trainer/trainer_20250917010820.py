...
#### 2. Mixed precision (예: bf16)과 gradient accumulation → Accelerate-> 이게 실제로 적용됨.
    ...
    def _save_best_local_only(self, cur_acc: float):
        """cfg.eval.local_only가 True일 때, 각 클라의 best checkpoint 저장."""
        if not getattr(self, "local_only", False):
            return
        ...
        try:
            ...
            # 저장 경로 및 파일명
            ckpt_root = "./checkpoinpts_1.0/local_only"   # ← 네가 지정한 디렉터리명 그대로 사용
            ...
            ckpt_path = os.path.join(ckpt_root, f"local_only_client_{cid:03d}.ckpt")
            meta_path = os.path.join(ckpt_root, f"local_only_client_{cid:03d}.json")

            # 모델 언랩(Accelerate/DDP 래핑 해제)
            base = self._unwrap(getattr(self.ctx, "model", None))
            if base is None:
                raise RuntimeError("No model to save (ctx.model is None).")

            # LoRA/PEFT가 있으면 어댑터만 저장, 아니면 전체 저장
            try:
                from peft import get_peft_model_state_dict
                state_to_save = get_peft_model_state_dict(base)
                logger.info("[CKPT] saving PEFT adapter-only state_dict")
            except Exception:
                state_to_save = base.state_dict()
                logger.info("[CKPT] PEFT export unavailable; saving full state_dict")
            ...
            logger.info(f"[CKPT] saved best checkpoint -> {ckpt_path}")
        except Exception as e:
            logger.warning(f"[CKPT] save failed: {e}")
            ...
            for sp in ['val','test']:
                ...
                if self._ct_ft and sp == 'test' and self._es_enabled and isinstance(out, dict):
                    ...
                    if cur_acc > (self._es_best + self._es_min_delta):
                        ...
                        self._save_best_local_only(cur_acc)
                    else:
                        self._es_wait += 1
                        if is_main:
