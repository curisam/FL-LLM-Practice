...
class LLMTrainer(GeneralTorchTrainer):
    """
    - Accelerate/Deepspeed/Vanilla Pytorch 지원
    - Reward-choice 데이터(샘플당 다중 시퀀스)에서도 '논리 배치 크기'로 집계
    - 배치별 n_choices 자동 추정
    """

    def __init__(self, model, data, device, config, only_for_eval=False, monitor=None):
        num_train_batch = len(data['train'])
        # 누적 스텝(상한: 한 epoch 배치 수)
        ...
        # 논리 마이크로 배치 목표 크기(사용자 설정 batch_size)
        self.target_micro_bs = int(getattr(config.dataloader, "batch_size", 1))

        # (fallback) 설정으로부터 n_choices 힌트 (없어도 OK, 실제 사용은 배치별 자동추정)
    ...
    def _run_batch(self, hooks_set, run_step: int = -1):
        ...
        if self.ctx.cur_mode in [MODE.TRAIN, MODE.FINETUNE]:
            if run_step == -1:
                ...
                if hasattr(self, 'accelerator') and self.accelerator is not None:
                    # 가속기 경로: 외부 루프가 '마이크로 배치 수'만큼 돈다
                    run_step = num_updates * max(1, grad_accum_step)
                else:
                    # 수동 누적: 외부는 '업데이트 수', 내부가 grad_accum_step 만큼 마이크로 처리
        ...
        if not hasattr(self.ctx, "debug_obs_micro_bs"):
            self.ctx.debug_obs_micro_bs = None
        if not hasattr(self.ctx, "debug_inferred_choices"):
            self.ctx.debug_inferred_choices = None
        ...
        for update_i in range(run_step):
            ...
            if using_accel:
                ...
                if self.ctx.debug_obs_micro_bs is None:
                    self.ctx.debug_obs_micro_bs = int(self._safe_int(getattr(self.ctx, "batch_size", 0), 0))
                    logger.info(
                        f"[{split}|accelerate] obs_micro_bs={self.ctx.debug_obs_micro_bs}, "
                        f"target_micro_bs={self.target_micro_bs}, "
                        f"inferred_n_choices={self.ctx.debug_inferred_choices}"
                    )
                ...
                for k in range(grad_accum_step):
                    ...
                    if self.ctx.debug_obs_micro_bs is None:
                        self.ctx.debug_obs_micro_bs = int(self._safe_int(getattr(self.ctx, "batch_size", 0), 0))
                        logger.info(
                            f"[{split}|vanilla] obs_micro_bs={self.ctx.debug_obs_micro_bs}, "
                            f"target_micro_bs={self.target_micro_bs}, "
                            f"inferred_n_choices={self.ctx.debug_inferred_choices}"
                        )
    ...
    def _infer_n_choices_from_batch(self, batch, raw_bs: int) -> int:
        ...
        2) ctx/config 힌트 사용
        3) 휴리스틱: raw_bs와 target_micro_bs의 비로 추정 (2~8 범위)
        실패 시 1 반환(보수적)
        """
        # 1) 명시 키
        try:
        ...
        for cand in range(2, 9):
            if raw_bs % cand == 0:
                logical = raw_bs // cand
                # target_micro_bs와 일치하거나 근접하면 채택
    ...
    def _hook_on_fit_start_init(self, ctx):
        ...
        ctx.debug_obs_micro_bs = None
        ctx.debug_inferred_choices = None
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        # --- 배치별 n_choices 자동 추정 & 논리 배치 크기 산출 ---
        ...
        if inferred >= 1 and raw_bs % inferred == 0:
            logical_bs = raw_bs // inferred
        else:
            # 실패 시 보수적으로 raw_bs 사용 (경고)
            logical_bs = raw_bs
            logger.warning(
                f"[logical-batch warn] raw_bs={raw_bs}, inferred_n_choices={inferred} → use raw_bs as logical."
            )
    ...
    def _hook_on_fit_end(self, ctx):
        ...
        if ctx.cur_mode in [MODE.TRAIN, MODE.FINETUNE]:
            ...
            logger.info(
                ...
                f"obs_micro_bs={getattr(ctx, 'debug_obs_micro_bs', None)}, "
                f"inferred_n_choices={getattr(ctx, 'debug_inferred_choices', None)}, "
                f"target_micro_bs={self.target_micro_bs}"
            )
            if int(ctx.num_samples) != int(expected_local_total):
                logger.warning(
                    ...
                    f"Check dataloader batch_size / choice grouping / sharding."
                )
        else:
            logger.info(
    ...
    def _hook_on_fit_end_free_space(self, ctx):
        attributes_to_delete = [
            ...
            'debug_done_micro', 'debug_obs_micro_bs', 'debug_inferred_choices',
        ]
