...
#### 2. Mixed precision (예: bf16)과 gradient accumulation → Accelerate-> 이게 실제로 적용됨.
    ...
    def _run_batch(self, hooks_set, run_step=-1): #한 epoch 내에서 batch 1개 도는 걸 여러번 반복. run_epoch에서는 run_step=-1 적용. 다른 곳에서는 run_step=1 적용.
        ...
        if self.ctx.cur_mode in [MODE.TRAIN, MODE.FINETUNE]:
            ...
            grad_accum_step = 1  #EVAL 모드면 누적 없이 한 배치씩 처리. grad_accum_step = 1로 강제하지만, 이 경우엔 어차피 backward/step을 호출하지 않으므로 accumulate의 동작이 사실상 아무 일도 안 합니다. 평가는 len(loader) 번 딱 한 바퀴 돌고 끝납니다.
    ...
    def _hook_on_batch_backward(self, ctx): #accelerate, deepspeed에 따른 backward, step, zero_grad 처리 분기
        ...
        if ctx.optimizer is not None:
            try:
                print(f"[DEBUG][Round {ctx.cur_round_i}, Batch {ctx.cur_batch_i}] "
                    f"LR={ctx.optimizer.param_groups[0]['lr']:.6e}, "
                    f"OptID={id(ctx.optimizer)}")
            except Exception as e:
                print(f"[DEBUG] Failed to read optimizer lr: {e}")
