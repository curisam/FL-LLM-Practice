...
class RewardChoiceTrainer(LLMTrainer): #LLMTrainer를 상속 받아, “여러 선택지 중 정답을 고르는” 형태의 LLM 파인튜닝 과제를 지원하도록 만든 커스텀 트레이너입니다.
    ...
    def _hook_on_batch_end(self, ctx):
        """
        매 배치가 끝난 후, 모든 모드(train/val/test)에 대해 
        결과를 ctx의 동적 변수(예: ctx.loss_total_train)에 누적합니다.
        """
        ...
        # getattr과 setattr을 사용하여 동적 변수에 접근
        # 예: ctx.loss_total_train += ...
        current_loss_total = getattr(ctx, loss_total_key)
        setattr(ctx, loss_total_key, current_loss_total + ctx.loss_batch.item() * batch_size)

        current_correct = getattr(ctx, correct_key)
        pred = torch.argmax(ctx.y_prob, dim=-1)
        setattr(ctx, correct_key, current_correct + (pred == ctx.y_true).sum().item())

        current_num_samples = getattr(ctx, num_samples_key)
        setattr(ctx, num_samples_key, current_num_samples + batch_size)

        # 4. 평가 모드일 때만 ys_true/ys_pred를 리스트에 쌓음
