...
class RewardChoiceTrainer(LLMTrainer):
    ...
    # LR 제어 로직을 다시 _hook_on_fit_start_init으로
    def _hook_on_fit_start_init(self, ctx):
        # 1. 부모 클래스 훅을 먼저 호출하여 ctx.optimizer를 생성시킴 (가장 중요)
        super()._hook_on_fit_start_init(ctx)
        ...
        if scheduler_cfg:
            ...
            if opt:
                ...
                # 이 경고는 이제 나타나지 않을 것임
                logger.warning("[Stateless LR Controller] Optimizer not found in ctx. Could not set LR.")
        # ------------------- LR 제어 로직 끝 -------------------
