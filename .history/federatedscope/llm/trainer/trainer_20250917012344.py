...
#### 2. Mixed precision (예: bf16)과 gradient accumulation → Accelerate-> 이게 실제로 적용됨.
    ...
    def _save_best_local_only(self):
        """LoRA 전용(학습가능 파라미터) 체크포인트를 클라이언트/스텝 단위로 저장."""
        ...
        step = int(getattr(self, "_global_updates", 0))   # ← 현재 optimizer step

        save_dir = "./checkpoints_1.0_local_only"
        os.makedirs(save_dir, exist_ok=True)

        # 예: local_only_client_033_step000480.ckpt
        save_path = os.path.join(save_dir, f"local_only_tldr_choice_qwen_client_{cid:03d}_step{step:06d}.ckpt")

        base = self._unwrap(self.ctx.model)
        sd = base.state_dict()        # LoRA(trainable)만 저장 (AdapterModel.state_dict 기본값)
        torch.save(sd, save_path)
        logger.info(f"[local-only] saved LoRA ckpt -> {save_path}")
