...
class EvalShardSampler(Sampler):
    def __init__(self, dataset_len: int, rank: int, world_size: int):
        self.indices = list(range(rank, dataset_len, world_size))
    def __iter__(self):
        return iter(self.indices)
    def __len__(self):
        return len(self.indices)
...
class RewardChoiceTrainer(LLMTrainer):
    def __init__(self, model, data, device, config, only_for_eval=False, monitor=None):
        super().__init__(model, data, device, config, only_for_eval, monitor)
        # Prepare choice token IDs
        choices_list = []
        for choice in config.trainer.choices:
            ids = self.tokenizer(f': {choice}', add_special_tokens=False)['input_ids']
            if not ids:
                raise ValueError(f"Empty token list for choice '{choice}'")
            choices_list.append(ids[-1])
        self.choices_cpu = torch.tensor(choices_list, dtype=torch.long)
        logger.info(f'Choice token IDs: {self.choices_cpu.tolist()}')

    def _reset_and_build_dataloader(self, split_name):
        data_key = f"{split_name}_data"
        client_data = self.ctx.data
        if split_name in client_data:
            del client_data[split_name]
        if hasattr(client_data, data_key) and getattr(client_data, data_key) is not None:
            dataset = WrapDataset(getattr(client_data, data_key))
            base_loader = get_dataloader(dataset, self.cfg, split_name)
            dist_ready, world_size, rank = _get_dist_info()
            if world_size > 1:
                if split_name == 'train':
                    ...
                    self._train_dist_sampler = sampler
                    loader = DataLoader(dataset=base_loader.dataset,
                                        batch_size=base_loader.batch_size,
                                        sampler=sampler,
                ...
                loader = base_loader
                local_count = len(base_loader.dataset)
            client_data[split_name] = loader
            setattr(self.ctx, f"{split_name}_loader", loader)
            logger.info(f"Dataloader for '{split_name}' reset: local_count={local_count}, total={len(base_loader.dataset)}")

    @use_diff
    def train(self, target_data_split_name="train", hooks_set=None):
        # Decay learning rate once per FL round
        if hasattr(self, 'scheduler'):
            self.scheduler.step()
            lr = self.scheduler.get_last_lr()[0]
            logger.info(f"[LRScheduler] After FL round → lr={lr:.2e}")
        self._reset_and_build_dataloader(target_data_split_name)
        return super().train(target_data_split_name, hooks_set)
    ...
    def evaluate(self, target_data_split_name="test", hooks_set=None):
        ...
        with torch.no_grad():
            if self.ctx.check_split(target_data_split_name, skip=True):
                self._run_routine(MODE.TEST, hooks_set or self.hooks_in_eval, target_data_split_name)
            else:
    ...
    def _hook_on_fit_start_init(self, ctx):
        super()._hook_on_fit_start_init(ctx)
        self.choices = self.choices_cpu.to(ctx.device)
        # Initialize scheduler
        sched_cfg = getattr(self.cfg.train, 'scheduler', None)
        if sched_cfg and sched_cfg.type == 'MultiStepLR':
            opt = getattr(ctx, 'optimizer', None) or getattr(self, 'optimizer', None)
            if opt:
                self.scheduler = MultiStepLR(opt,
                                             milestones=sched_cfg.milestones,
                                             gamma=sched_cfg.gamma,
                                             last_epoch=-1)
            else:
                logger.warning("[LRScheduler] Optimizer not found; scheduler disabled.")

    def _hook_on_batch_end(self, ctx):
        skip = getattr(ctx, 'skip_this_batch', False)
        if skip:
            return
        # Fetch current lr and log
        opt = getattr(self, 'optimizer', None) or getattr(ctx, 'optimizer', None)
        if not opt and hasattr(self, 'accelerator'):
            try:
                opt = self.accelerator._optimizer_groups[0]['optimizer']
            except:
                pass
        if opt:
            current_lr = opt.param_groups[0]['lr']
            round_idx = getattr(ctx, 'cur_state', getattr(ctx, 'round_idx', '?'))
            batch_idx = getattr(ctx, 'batch_idx', '?')
            logger.info(f"[LR] round={round_idx} batch={batch_idx} lr={current_lr:.2e}")
