...
#### 2. Mixed precision (예: bf16)과 gradient accumulation → Accelerate-> 이게 실제로 적용됨.
    ...
    def _run_batch(self, hooks_set, run_step=-1):
        ...
        # 3) 반복 횟수는 "현재 로더의 실제 길이" 기준
        total_micro_batches = len(loader)
        if run_step == -1:
            if (self._cfg.train.batch_or_epoch == 'epoch'
                    and self.ctx.cur_mode in [MODE.TRAIN, MODE.FINETUNE]):
                # 한 업데이트 스텝에 grad_accum_step개 미니배치를 소모
                run_step = math.ceil(total_micro_batches / max(1, grad_accum_step))
            else:
                # batch 모드 또는 eval 모드 → 미니배치 개수만큼
                run_step = total_micro_batches
        ...
        for update_i in range(run_step):
            ...
            if hasattr(self, 'accelerator'):  # accelerate 모드
    ...
    def _hook_on_batch_forward(self, ctx): #ctx.data_batch → input_ids, attention_mask, labels로 명시 처리 + 모델 호출 방식 변경. 기존 toprch trainer는 CNN, MLP, BERT 용이라 LLM용은 아니라서 OVERRIDE 해야함.
        ...
        if ctx.cfg.llm.accelerator.use: #참고
            ...
            outputs = ctx.model(input_ids=input_ids,
                                labels=labels,
                                attention_mask=attention_mask).to(ctx.device) # 자동으로 모델과 배치 텐서 모두 디바이스로 이동
    ...
    def _hook_on_batch_backward(self, ctx): #accelerate, deepspeed에 따른 backward, step, zero_grad 처리 분기
        ...
        if ctx.cfg.llm.accelerator.use: #TRUE
            self.accelerator.backward(ctx.loss_task)#→ Mixed-precision(DP/BF16), DDP 동기화 등을 고려해 gradient를 계산
            ctx.optimizer.step()
