...
class EvalShardSampler(Sampler):
    """테스트/밸리데이션 전용 샘플러.
    - 중복/패딩 없이 rank마다 고유 인덱스만 반환
    - 총합 == 데이터셋 길이 보장
    - 각 랭크 샘플 수는 floor/ceil(N/world_size)
    """
    def __init__(self, dataset_len: int, rank: int, world_size: int):
        self.dataset_len = int(dataset_len)
        self.rank = int(rank)
        self.world_size = int(world_size)
        # 예: rank, rank+world_size, rank+2*world_size, ...
        self.indices = list(range(self.rank, self.dataset_len, self.world_size))
...
class RewardChoiceTrainer(LLMTrainer): #LLMTrainer를 상속 받아, “여러 선택지 중 정답을 고르는” 형태의 LLM 파인튜닝 과제를 지원하도록 만든 커스텀 트레이너입니다.
    ...
    # def _reset_and_build_dataloader(self, split_name):
    #     data_key = f"{split_name}_data"
    #     loader_key = split_name  # ClientData는 'train'/'val'/'test' 키 사용
    #     ctx_loader_key = f"{split_name}_loader"
        
    #     client_data_obj = self.ctx.data  # ClientData 객체
        
    #     # 1) ClientData 내부의 기존 로더 제거 (참조 루트 끊기)
    #     if loader_key in client_data_obj:
    #         del client_data_obj[loader_key]
        
    #     # 2) 원본 데이터를 사용해서 새로운 로더 생성
    #     if hasattr(client_data_obj, data_key) and getattr(client_data_obj, data_key) is not None:
    #         dataset = WrapDataset(getattr(client_data_obj, data_key))

    #         # 기본 로더를 먼저 받음(여기서 collate_fn 등 구성 가져오려고)
    #         base_loader = get_dataloader(dataset, self.cfg, split_name)

    #         # --- 멀티프로세스라면 평가용 샤딩 적용(테스트/밸리데이션만) ---
    #         multi_proc = (
    #             hasattr(self, 'accelerator') 
    #             and self.accelerator is not None 
    #             and getattr(self.accelerator, 'num_processes', 1) > 1
    #         )
    #         if split_name in ['test', 'val'] and multi_proc:
    ...
    #                 drop_last=False
    #             )
    #             # base_loader에서 설정을 복사하여 새 DataLoader 구성
    #             loader = DataLoader(
    #                 dataset=base_loader.dataset,
    #                 batch_size=base_loader.batch_size,
    #                 sampler=sampler,
    ...
    #                     f"(sharded={split_name in ['test','val'] and multi_proc})")
    ...
    def _hook_on_batch_end(self, ctx):
        ...
        # 3. 현재 배치의 결과를 가져와서 동적 변수에 누적
        batch_size = len(ctx.data_batch['input_ids'])
        
        ctx[num_samples_key] += batch_size
        ctx[loss_total_key] += ctx.loss_batch.item() * batch_size
        
        pred = torch.argmax(ctx.y_prob, dim=-1)
        ctx[correct_key] += (pred == ctx.y_true).sum().item()

        # 4. 평가 모드일 때만 ys_true/ys_pred를 "리스트"에 쌓습니다.
