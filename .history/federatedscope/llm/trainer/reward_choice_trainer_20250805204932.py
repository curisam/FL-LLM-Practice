...
class RewardChoiceTrainer(LLMTrainer):
    ...
    @use_diff
    def train(self, target_data_split_name="train", hooks_set=None, round_num=-1):
        # [수정 1] Client에서 받은 라운드 번호를 ctx에 저장
        self.ctx.current_round_num = round_num
    ...
    def _hook_on_fit_start_init(self, ctx):
        ...
        if hasattr(ctx, "new_lr_to_be_set"):
            ...
            if hasattr(ctx, 'optimizer') and ctx.optimizer is not None:
                ...
                logger.warning("Optimizer was not ready, could not apply the planned LR.")
        
        # 라운드 번호는 train()에서 받은 것을 사용하거나, 없으면 cur_state 사용
    ...
    def _hook_on_batch_end(self, ctx):
        ...
        if ctx.cur_mode == MODE.TRAIN:
            ...
            if opt is not None:
                ...
                # train()에서 저장한 정확한 라운드 번호 사용
                round_idx = getattr(ctx, "current_round_num", "?")
                batch_idx = getattr(ctx, "batch_idx", "?")
                # logger.debug 대신 logger.info 사용하여 항상 보이도록 함
                logger.info(f"[LR BATCH CHECK] round={round_idx} batch={batch_idx} -> LR={current_lr:.2e}")
            else:
                logger.warning(f"[LR BATCH CHECK] round={round_idx} batch={batch_idx} -> Optimizer not found!")
