...
class RewardChoiceTrainer(LLMTrainer): #LLMTrainer를 상속 받아, “여러 선택지 중 정답을 고르는” 형태의 LLM 파인튜닝 과제를 지원하도록 만든 커스텀 트레이너입니다.
    ...
    def _hook_on_batch_end(self, ctx):
        ...
        # ctx에 누적 변수가 없으면 0으로 초기화 (최초 한 번만 실행됨)
        # LLMTrainer의 _hook_on_fit_start_init에서 이미 초기화할 수 있지만, 안전을 위해 여기서도 확인
        ...
        # 현재 배치의 총 손실 (평균 손실 * 배치 크기)을 누적
        batch_size = len(ctx.data_batch['input_ids'])
        ctx.loss_total += ctx.loss_batch.item() * batch_size

        # 현재 배치의 정답 개수를 누적
        pred = torch.argmax(ctx.y_prob, dim=-1)
        # ctx.y_true는 _hook_on_batch_forward에서 생성된 1차원 텐서
        correct_in_batch = (pred == ctx.y_true).sum().item()
        ctx.correct += correct_in_batch

        # 샘플 수 누적 (BaseTrainer나 LLMTrainer에 이미 있을 수 있으나, 여기서 명시적으로 처리)
        # ctx.num_samples는 _run_epoch에서 관리하므로 여기서는 건드리지 않는 것이 안전할 수 있습니다.
        # _run_epoch의 for 루프가 ctx.num_train_batch를 사용하므로, num_samples는 batch_size의 합과 같습니다.

        # 평가 모드일 때만 ys_true/ys_pred를 리스트에 쌓습니다.
        if ctx.cur_mode in [MODE.TEST, MODE.VAL]:
        ...
        # 부모 클래스(LLMTrainer)의 _hook_on_batch_end가 있다면 호출
        # LLMTrainer의 코드를 보니, num_samples와 loss_batch_total을 누적하므로
        # 중복을 피하기 위해 super() 호출을 제거하거나, 위의 누적 로직을 제거해야 합니다.
        # 여기서는 명시적으로 직접 구현했으므로 super()를 호출하지 않는 것이 더 안전합니다.
        # super()._hook_on_batch_end(ctx)
