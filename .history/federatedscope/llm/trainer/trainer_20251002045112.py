...
#### 2. Mixed precision (예: bf16)과 gradient accumulation → Accelerate-> 이게 실제로 적용됨.
    ...
    #     # 디버깅을 위해 모델의 device 확인
    #     if using_accel:
    #         device = self.accelerator.device
    #         model = self.model.eval()
    #         self.accelerator.wait_for_everyone()
    #         model = self.accelerator.prepare(model)  # accelerator로 모델 준비
    #     else:
    #         model = self.model
    ...
    #     test_loader = self.ctx.data['test']
    #     if using_accel:
    #         test_loader = self.accelerator.prepare(test_loader)

    #     # === choices 텐서를 device로 이동 ===
    #     self.choices = self.choices_cpu.to(device, dtype=torch.long, non_blocking=True)
    ...
    #             # shift_logits와 shift_labels가 동일한 device에 있는지 확인
    ...
    def _mid_eval_once_stat(self):
        """
        전 프로세스(랭크)에서 샘플별 (p_A, 라벨)을 수집한 뒤,
        메인 프로세스가 40줄짜리 CSV를 한 번만 기록.
        """
        import os, csv, torch
        import torch.nn.functional as F
        ...
        # 1) 이미 준비된 모델/디바이스 사용(재-prepare 금지)
        model = getattr(getattr(self, "ctx", object()), "model", None) or getattr(self, "model", None)
        assert model is not None, "model not found"
        model.eval()
        device = self.accelerator.device if using_accel else next(model.parameters()).device

        # 2) dataloader 재빌드 (+ 데이터만 prepare)
        self._reset_and_build_dataloader(split_name='test')
        test_loader = self.ctx.data['test']
        if using_accel:
            test_loader = self.accelerator.prepare(test_loader)
        ...
        # (옵션) 글로벌 인덱스 복원: EvalShardSampler는 indices=[rank, rank+world, ...]
        # DataLoader가 sampler 순서대로 꺼내므로 순차 카운터로 매칭 가능
        sampler = getattr(test_loader, "sampler", None)
        local_indices = getattr(sampler, "indices", None)
        use_global_idx = isinstance(local_indices, list)
        local_ptr = 0  # local_indices를 소비할 포인터

        # 4) 랭크 로컬 버퍼 (리스트로 모아 gather_object 사용)
        triplets_local = []  # (global_idx, pA, y) 또는 (seq_id, pA, y)
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                first_pos = valid.float().cumsum(dim=1).eq(1).argmax(dim=1)
                bidx = torch.arange(Bp, device=device)

                logits_ab    = torch.index_select(shift_logits, dim=-1, index=choices_ids)  # [Bp,T-1,2]
                logits_first = logits_ab[bidx, first_pos, :]
                probs_ab     = F.softmax(logits_first, dim=-1)
                pA           = probs_ab[:, 0]  # [Bp]
                y            = (shift_labels[bidx, first_pos] == choices_ids[0]).float()  # A=1,B=0

                # 글로벌 인덱스(정렬용)
                if use_global_idx:
                    # 이 배치의 샘플이 sampler.indices의 local_ptr..local_ptr+Bp-1에 대응
                    gidx_batch = local_indices[local_ptr: local_ptr + Bp]
                    local_ptr += Bp
                else:
                    # sampler를 모를 경우, (rank, 누적 카운터)로라도 순서 재현 가능
                    gidx_batch = list(range(local_ptr, local_ptr + Bp))
                    local_ptr += Bp

                # 리스트에 append
                pa_list = pA.detach().cpu().tolist()
                y_list  = y.detach().cpu().tolist()
                triplets_local.extend(zip(gidx_batch, pa_list, y_list))

        # 5) 전 랭크 수집 → 메인만 기록
        if using_accel and world > 1:
            all_triplets = self.accelerator.gather_object(triplets_local)
            # gather_object는 랭크별 리스트들의 리스트를 반환
            if is_main:
                merged = [t for sub in all_triplets for t in sub]
        else:
            merged = triplets_local

        if is_main:
            # 글로벌 인덱스로 정렬 후, 0..N-1로 재라벨링
            merged.sort(key=lambda x: x[0])
            # (혹시 샘플 수가 40 미만이면 sampler/배치 구성 문제이므로 경고)
            N = len(merged)
            cid = int(getattr(getattr(self, "ctx", object()), "client_ID", 0))
            if N != 40:
                logger.warning(f"[STAT-CSV] client={cid} gathered {N} samples (expected 40)")
