...
class RewardChoiceTrainer(LLMTrainer):
    ...
    def _hook_on_fit_start_init(self, ctx):
        ...
        # ==================== 디버깅 코드 시작 ====================
        logger.info("==================== CTX DEBUG START ====================")
        # ctx는 딕셔너리처럼 동작하는 객체입니다. 모든 key-value를 출력해봅니다.
        for key, value in ctx.items():
            # 너무 긴 값은 잘라서 출력
            value_str = str(value)
            if len(value_str) > 200:
                value_str = value_str[:200] + "..."
            logger.info(f"CTX_DEBUG | {key}: {value_str}")
        logger.info("==================== CTX DEBUG END ======================")
        # ==================== 디버깅 코드 끝 ======================
        
        # Optimizer는 학습 모드에서만 생성되므로, 존재하는지 먼저 확인
        if hasattr(ctx, 'optimizer') and ctx.optimizer is not None:
            # `cur_state`는 client.state를 가리키며 서버가 업데이트해주는 값입니다.
