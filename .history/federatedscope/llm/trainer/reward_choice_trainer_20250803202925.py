...
class RewardChoiceTrainer(LLMTrainer):
    ...
    def _hook_on_fit_end(self, ctx):
        """
        - 샘플 수/로스 합산: 부모 배치 훅이 유지한 ctx.num_samples, ctx.loss_batch_total 기반
        - 모든 프로세스 합(reduce) 후, {split}_total / {split}_loss / {split}_avg_loss 기록
        - 우리가 누적해 둔 sample 기준 정확도도 reduce 후 {split}_acc 기록
        """
        split = getattr(ctx, "cur_split", None)
        if split not in ("train", "val", "test"):
            return

        # 1) 로컬 누적치(부모가 배치마다 갱신): 논리 샘플 기준 수치
        local_n    = int(getattr(ctx, "num_samples", 0))
        local_loss = float(getattr(ctx, "loss_batch_total", 0.0))

        # 2) 전 프로세스 합산
        acc   = getattr(self, "accelerator", None)
        device = ctx.device
        t_n    = torch.tensor([local_n], device=device, dtype=torch.long)
        t_loss = torch.tensor([local_loss], device=device, dtype=torch.float32)
        if acc is not None and acc.num_processes > 1:
            t_n    = acc.reduce(t_n, reduction="sum")
            t_loss = acc.reduce(t_loss, reduction="sum")
        n_global    = int(t_n.item())
        loss_global = float(t_loss.item())
        avg_global  = float(loss_global / max(n_global, 1))

        # 3) 정확도(샘플 기준)도 reduce
        local_seen = int(getattr(ctx, "sample_seen", 0))
        local_corr = int(getattr(ctx, "sample_correct", 0))
        t_seen = torch.tensor([local_seen], device=device, dtype=torch.long)
        t_corr = torch.tensor([local_corr], device=device, dtype=torch.long)
        if acc is not None and acc.num_processes > 1:
            t_seen = acc.reduce(t_seen, reduction="sum")
            t_corr = acc.reduce(t_corr, reduction="sum")
        ...
        # 4) 메트릭 기록 (rank0만 쓰도록; 나머지는 빈 dict)
        ...
        logger.info(
            ...
            f"(local_seen={local_seen}, local_corr={local_corr})"
        )
