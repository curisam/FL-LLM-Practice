...
class RewardChoiceTrainer(LLMTrainer):
    ...
    @lifecycle(LIFECYCLE.EPOCH_END)
    def _hook_on_epoch_end_step_lr(self, ctx):
        """매 에폭이 끝날 때마다 scheduler.step() 호출"""
        if hasattr(self, 'scheduler'):
            # MultiStepLR 을 쓰고 있다면
            self.scheduler.step()
            # get_last_lr()는 리스트를 반환하므로 첫 번째 LR만 꺼냅니다.
            lr = self.scheduler.get_last_lr()[0]
            logger.info(f"[LRScheduler] After epoch {ctx.cur_epoch}, lr={lr:.2e}")
