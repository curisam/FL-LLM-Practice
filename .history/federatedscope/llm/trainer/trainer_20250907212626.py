...
#### 2. Mixed precision (예: bf16)과 gradient accumulation → Accelerate-> 이게 실제로 적용됨.
    ...
    def _mid_eval_once(self):
        """CT-FT에서만, 현재 업데이트 시점의 mid-eval(VAL/TEST)을 '기존 evaluate()' 경로로 실행."""
        # (선택) 분산이면 배리어
        if hasattr(self, 'accelerator') and self.accelerator is not None:
            self.accelerator.wait_for_everyone()
        ...
        for sp in set(getattr(self.cfg.eval, "split", [])):   # 예: ['val','test']
            out = self.evaluate(target_data_split_name=sp)    # ← 네 기존 evaluate() 그대로 호출
            results[sp] = out

        # rank0(또는 단일 프로세스)에서만 기록
        ...
        if is_main:
            ...
            # ctx.eval_metrics는 예: val_total/val_avg_loss/test_avg_loss … 형태
            flat = {}
            ...
            with open(path, "a", encoding="utf-8") as f:
                f.write(json.dumps({
                    "round": rnd,
                    "step": self._global_updates,   # '업데이트 수' 기준 스텝
                    "phase": "mid",
                    **flat,
                }, ensure_ascii=False) + "\n")
