...
class RewardChoiceTrainer(LLMTrainer): #LLMTrainer를 상속 받아, “여러 선택지 중 정답을 고르는” 형태의 LLM 파인튜닝 과제를 지원하도록 만든 커스텀 트레이너입니다.
    ...
    def _hook_on_batch_end(self, ctx):
        ...
        # 안전 초기화
        if not hasattr(ctx, loss_total_key):   setattr(ctx, loss_total_key, 0.0)
        if not hasattr(ctx, correct_key):      setattr(ctx, correct_key, 0.0)  # float 누적
        if not hasattr(ctx, num_samples_key):  setattr(ctx, num_samples_key, 0.0)

        # === 핵심: 유효 토큰 수(n_valid) 기준으로만 누적 ===
        n_valid = ctx.y_true.numel()  # IGNORE_INDEX 제거 후 남은 토큰 수 (= 한 샘플에 1~여러 개)

        # gradient accumulation 보정
        accum = int(getattr(self.cfg.llm, "grad_accum_step", 1))
        accum = max(1, accum)

        # 손실: 보통 평균(loss) 이므로 유효 샘플 수로 가중한 뒤 accum으로 나눠서 "optimizer step 기준"으로 정규화
        setattr(ctx, loss_total_key,
                getattr(ctx, loss_total_key) + float(ctx.loss_batch.item()) * (n_valid / float(accum)))

        # 정확도 분자: 맞춘 개수
        pred    = torch.argmax(ctx.y_prob, dim=-1)  # [n_valid]
        correct = float((pred == ctx.y_true).sum().item()) / float(accum)
        setattr(ctx, correct_key, getattr(ctx, correct_key) + correct)

        # 정확도 분모: 유효 샘플 수
        setattr(ctx, num_samples_key, getattr(ctx, num_samples_key) + (n_valid / float(accum)))

        # (옵션) 평가 모드에서만 원시 예측 저장
        if ctx.cur_mode in [MODE.TEST, MODE.VAL]:
            if not hasattr(ctx, 'ys_true'): ctx.ys_true = []
            if not hasattr(ctx, 'ys_pred'): ctx.ys_pred = []
            ctx.ys_true.append(ctx.y_true)
    ...
    def _hook_on_fit_end(self, ctx):
        ...
        if split in ["train", "test", "val"]:
            ...
            ctx.total = int(round(n))                 # ← per-rank는 보통 120, 집계 480 기대
            ctx.loss = loss_total
