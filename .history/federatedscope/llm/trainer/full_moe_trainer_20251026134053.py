...
class FullMoETrainer(LLMTrainer):
    ...
    def _hook_on_batch_forward(self, ctx) -> None:
        ...
        for idx, adapter_name in enumerate(self.adapter_names):
            ...
            weighted_correct += weight * sample_correct
            weighted_count += weight * sample_count
            ...
            if weighted_y_prob is None:
                ...
                weighted_y_true = flat_labels  # 동일한 flat_labels로 가정
            else:
        ...
        # ctx.loss_task와 통계 설정
        ctx.loss_task = weighted_loss
        ctx.loss_batch = CtxVar(weighted_loss.item(), LIFECYCLE.BATCH)
        ctx.loss = weighted_loss.item()
        ctx.batch_size = CtxVar(len(labels), LIFECYCLE.BATCH)

        ctx.sample_correct_batch = int(weighted_correct)
        ctx.sample_count_batch = float(weighted_count)

        if weighted_y_prob is not None:
            predicted = torch.argmax(weighted_y_prob, dim=1)
            ctx.y_true = CtxVar(weighted_y_true.cpu(), LIFECYCLE.BATCH)
            ctx.y_pred = CtxVar(predicted.cpu(), LIFECYCLE.BATCH)
            ctx.y_prob = CtxVar(weighted_y_prob.cpu(), LIFECYCLE.BATCH)
        else:
            ctx.y_true = CtxVar(torch.tensor([], dtype=torch.long),
                                LIFECYCLE.BATCH)
            ctx.y_pred = CtxVar(torch.tensor([], dtype=torch.long),
                                LIFECYCLE.BATCH)
            ctx.y_prob = CtxVar(torch.tensor([], dtype=torch.float32),
                                LIFECYCLE.BATCH)
