...
class RewardChoiceTrainer(LLMTrainer):
    ...
    # --- train/eval 진입 시 로더 리빌드 (스케줄러 로직 없음) ---
    @use_diff
    def train(self, target_data_split_name="train", hooks_set=None):
        self._reset_and_build_dataloader(target_data_split_name)
        return super().train(target_data_split_name, hooks_set)
    ...
    # --- 라운드 시작 초기화 (LR 제어 로직 포함) ---
    def _hook_on_fit_start_init(self, ctx):
        # 부모 클래스의 초기화 루틴을 먼저 호출 (optimizer 등을 설정)
        super()._hook_on_fit_start_init(ctx)

        # ==================== Stateless LR 제어 로직 시작 ====================
        # 이 로직은 매 클라이언트 학습 시작 시마다 실행되어 LR을 올바르게 설정합니다.

        # 1. 현재 FL 라운드 번호를 가져옵니다 (0부터 시작).
        current_round = int(getattr(ctx, "cur_state", 0))

        # 2. YAML 파일로부터 스케줄러 설정을 읽어옵니다.
