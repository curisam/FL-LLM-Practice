...
class RewardChoiceTrainer(LLMTrainer): #LLMTrainer를 상속 받아, “여러 선택지 중 정답을 고르는” 형태의 LLM 파인튜닝 과제를 지원하도록 만든 커스텀 트레이너입니다.
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        # ---- 배치 콘텐츠 서명(signature) 생성 ----
        # 같은 텐서 객체가 아니어도, "같은 배치"면 서명이 같도록 내용 기반으로 생성
        try:
            ids_head = input_ids[:, :16].detach().to('cpu', non_blocking=True)
            lbs_head = labels[:, :16].detach().to('cpu', non_blocking=True)
            # (배치크기, input_ids 앞부분 합, labels 앞부분 합)
            batch_sig = (
                int(input_ids.size(0)),
                int(ids_head.sum().item()) & 0xFFFFFFFF,
                int(lbs_head.sum().item()) & 0xFFFFFFFF,
            )
        except Exception:
            # 만약 위가 실패하면 형태 기반 fallback
            batch_sig = (int(input_ids.size(0)), int(input_ids.numel()), int(labels.numel()))

        ctx._batch_signature = batch_sig
