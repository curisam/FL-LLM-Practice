...
class RewardChoiceTrainer(LLMTrainer):
    def __init__(self, model, data, device, config, only_for_eval=False, monitor=None):
        ...
        if hasattr(self, "accelerator"):
            self.optimizer = AdamW(self.model.parameters(), lr=config.train.optimizer.lr, betas=config.train.optimizer.betas)
            if sched_cfg and getattr(sched_cfg, "type", "") == "MultiStepLR":
                self.scheduler = MultiStepLR(self.optimizer, milestones=sched_cfg.milestones, gamma=sched_cfg.gamma)
                self.model, self.optimizer, self.scheduler = self.accelerator.prepare(self.model, self.optimizer, self.scheduler)
                logger.info(f"[LRScheduler] MultiStepLR 초기화: milestones={sched_cfg.milestones}, gamma={sched_cfg.gamma}")
            else:
    ...
    def _hook_on_fit_start_init(self, ctx):
        ...
        if hasattr(self, "scheduler") and self.scheduler is not None:
            round_idx = getattr(ctx, "cur_state", 0)
            logger.info(f"[LR DEBUG] 라운드 시작 - round_idx: {round_idx}")
            self.scheduler.step(round_idx)
            lr = self.scheduler.get_last_lr()[0]
            logger.info(f"[LRScheduler] 라운드 {round_idx} → lr={lr:.2e}, 스케줄러 상태: {self.scheduler.state_dict()}")
        else:
            logger.warning("[LRScheduler] 스케줄러가 초기화되지 않았습니다. 학습률이 갱신되지 않을 수 있습니다.")
    ...
    def _hook_on_batch_end(self, ctx):
        ...
        if hasattr(self, "accelerator"):
            try:
                ...
                pass
        if opt is None and hasattr(self, "optimizer"):
            opt = self.optimizer
        if opt is None and hasattr(ctx, "optimizer"):
            opt = ctx.optimizer
        ...
        if opt is not None:
            ...
            batch_idx = getattr(ctx, "batch_idx", "?")
            logger.debug(
                f"[LR DEBUG] 라운드={round_idx}, 배치={batch_idx}, split={ctx.cur_split}, lr={current_lr:.2e}"
            )
        else:
            logger.debug("[LR DEBUG] 옵티마이저를 찾을 수 없어 LR 로그를 출력하지 않았습니다.")

        # ... (나머지 _hook_on_batch_end 로직은 동일) ...
    ...
    def _hook_on_fit_end(self, ctx):
        ...
        if hasattr(self, "scheduler") and self.scheduler is not None:
            round_idx = getattr(ctx, "cur_state", 0)
            lr = self.scheduler.get_last_lr()[0]
            logger.info(f"[LRScheduler] 라운드 {round_idx} 종료 → lr={lr:.2e}, 스케줄러 상태: {self.scheduler.state_dict()}")

    def _hook_on_fit_end_free_space(self, ctx):
        # 메모리 정리 대상 속성 (scheduler와 optimizer는 제외)
        ...
        if hasattr(self, 'accelerator'):
            ...
            logger.info("[Memory Cleanup] Accelerator memory has been freed.")
