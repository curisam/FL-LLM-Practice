...
class RewardChoiceTrainer(LLMTrainer): #LLM이 뱉는 logits[B, T, V](V=전체 vocab)에 대해 “다음 토큰이 A냐 B냐만” 보도록 축소해서 **CrossEntropyLoss(B×(T-1) vs 2)**로 학습/평가한다. 
    ...
    def _hook_on_fit_start_init(self, ctx):
        ...
        ctx.num_samples_train = CtxVar(0, LIFECYCLE.ROUND)
        ctx.loss_total_train = CtxVar(0.0, LIFECYCLE.ROUND)
        ctx.correct_train    = CtxVar(0, LIFECYCLE.ROUND) 
