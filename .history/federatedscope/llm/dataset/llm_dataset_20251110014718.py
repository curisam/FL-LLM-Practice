...
class LLMDataset(Dataset):
    def __init__(self,
        ...
        type_names = [type(v).__name__ for v in categories]
        logger.info("[cat debug] type histogram: %s", Counter(type_names))
        ...
        for i, v in enumerate(categories):
            if isinstance(v, (list, tuple, np.ndarray)):
                ...
                # 0-d, numel==1 만 스칼라 취급
                if not (v.ndim == 0 and v.numel() == 1):
                    bad_idx.append(i)

        if bad_idx:
            logger.warning("[cat debug] non-scalar-like categories found: %d", len(bad_idx))
            for i in bad_idx[:5]:
                v = categories[i]
                shape = getattr(v, 'shape', None)
                size  = getattr(v, 'size', None)
                logger.warning("[cat debug] idx=%d type=%s shape=%s size=%s value(head)=%s",
                               i, type(v).__name__, str(shape), str(size), str(v)[:120])
        # --- 최소 디버깅 추가 끝 ---

        try:
            df = pd.DataFrame(categories, columns=["category"])
        except Exception as e:
            # 실패 시 무엇 때문에 죽는지 한 번 더 자세히 남김
            first_bad = bad_idx[0] if bad_idx else None
            v = categories[first_bad] if first_bad is not None else None
            shape = getattr(v, 'shape', None)
            size  = getattr(v, 'size', None)
            logger.exception("[cat debug] DataFrame build failed. first_bad_idx=%s "
                             "type=%s shape=%s size=%s value(head)=%s",
                             str(first_bad),
                             type(v).__name__ if v is not None else None,
                             str(shape), str(size), str(v)[:200])
            raise
