...
def load_llm_dataset(config=None, **kwargs):
    ...
    if dataset_name.endswith('.json'):
    ...
    elif dataset_name.lower() == 'reddit-tldr-comparison-choice': #TLDR
        from federatedscope.llm.dataloader.reddit_tldr import \
    ...
    elif dataset_name.lower() == 'shp-comparison': #SHP
        ...
        dataset = load_shp_cmp_dataset_by_choice(data_root,
                                                 tokenizer,
                                                 config,
                                                 max_num_test=1000)#(train_dataset, val_dataset, test_dataset)의 형태. 각각은 LLMDataset 클래스
