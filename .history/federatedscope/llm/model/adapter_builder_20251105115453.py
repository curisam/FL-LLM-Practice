...
class AdapterModel(nn.Module):
    ...
    def append_adapters_random(self, adapter_names, peft_config=None): 
        ...
        def _init_normal_safe(tensor, mean, std, gen):
            # Try using generator param (newer PyTorch). If not supported, fallback to
            # temporarily seeding global RNGs so init is reproducible per-adapter.
            try:
                torch.nn.init.normal_(tensor, mean=mean, std=std, generator=gen)
            except TypeError:
            ...
            with torch.no_grad():
                for m in self._iter_lora_modules():
                    ...
                    if hasattr(m, "lora_A") and name in getattr(m, "lora_A", {}):
                        _init_normal_safe(m.lora_A[name], mean=0.0, std=0.02, gen=gen)
                    if hasattr(m, "lora_B") and name in getattr(m, "lora_B", {}):
                        _init_normal_safe(m.lora_B[name], mean=0.0, std=0.02, gen=gen)
                    # 임베딩 타입도 있으면 처리
                    if hasattr(m, "lora_embedding_A") and name in getattr(m, "lora_embedding_A", {}):
                        _init_normal_safe(m.lora_embedding_A[name], mean=0.0, std=0.02, gen=gen)
                    if hasattr(m, "lora_embedding_B") and name in getattr(m, "lora_embedding_B", {}):
                        _init_normal_safe(m.lora_embedding_B[name], mean=0.0, std=0.02, gen=gen)
