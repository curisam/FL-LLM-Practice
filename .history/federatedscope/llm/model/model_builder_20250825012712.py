...
def get_llm(config, load_from_prev_ckpt=False, **kwargs):
    ...
    if config.llm.adapter.use and config.llm.adapter.local_only: # config.llm.adapter.local_only=False로 PASS!!
    ...
    if config.llm.adapter.count > 1: #3으로 적용되어서 걸림, 실질적으로 forward에 쓰이는 것은 1개뿐
        ...
        ])#"default", "Adapter_1", "Adapter_2", "Adapter_3"의 adapter가 있게 됨. Adapter class인 model은 append_adapters 존재.
