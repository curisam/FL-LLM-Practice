...
def enable_adapter(model, package, adapter, **kwargs):#package: peft, adapter:lora로 들어감.
    ...
    if package == 'peft':
        ...
        if adapter == 'lora': ########## 이거에 해당 ##############
            ...
            LoRA 어댑터(A/B 행렬)를 “끼워 넣었고”, 그 결과 state_dict 키에 base_model....lora_A/B...가 생기고, 
            베이스 가중치는 동결, LoRA 가중치만 학습/전송 대상으로 바뀐 것이야.           
            
            """

            """
            앞부분(prefix): PEFT가 래핑하면서 경로에 base_model 이 붙어요.
            ...
            *.lora_A.*, *.lora_B.*만 서버로 보내게 됨(베이스는 동결이므로 제외).
            ...
            model = get_peft_model(model, peft_config) #PEFT 클래스 객체. state_dict를 할때 backbone, lora 전부 포함되어서 나옴.
        elif adapter == 'prefix':
...
class AdapterModel(nn.Module):
    ...
    def get_trainable_state_dict(self): #현재 모델(self.model)에서 전송/저장을 위한 학습 대상 파라미터만 뽑아 OrderedDict로 반환.
        ...
        멀티 어댑터 보정: self.adapter_names에 있는 어댑터 이름이 파라미터 이름 문자열에 포함되면, requires_grad=False라도 포함      
        """
        grad_params = []
    ...
    def save_model(self,
                   ...
                   state=0,
                   merge_adapter=False,
                   return_trainable=True): #체크포인트를 파일로 저장. 케이스에 따라 ckpt = {'cur_round': state, 'model': ...}를 구성하고 torch.save(ckpt, path) 호출
