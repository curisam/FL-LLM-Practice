...
class AdapterModel(nn.Module):
    ...
    def append_adapters_random(self, adapter_names, peft_config=None): #여러 개의 어댑터를 동일한 peft_config로 추가할 준비.외부에서 별도 config 주지 않으면 현재 보유한 self.peft_config 재사용.
        assert isinstance(self.model, PeftModel)
        peft_config = self.peft_config if peft_config is None else peft_config
            ...
            with torch.no_grad():
                for m in self._iter_lora_modules():
                    ...
                    if hasattr(m, "lora_A") and name in getattr(m, "lora_A", {}):
                        torch.nn.init.normal_(m.lora_A[name], mean=0.0, std=0.02, generator=gen)
                    if hasattr(m, "lora_B") and name in getattr(m, "lora_B", {}):
                        torch.nn.init.normal_(m.lora_B[name], mean=0.0, std=0.02, generator=gen)
                    # 임베딩 타입도 있으면 처리
                    if hasattr(m, "lora_embedding_A") and name in getattr(m, "lora_embedding_A", {}):
                        torch.nn.init.normal_(m.lora_embedding_A[name], mean=0.0, std=0.02, generator=gen)
                    if hasattr(m, "lora_embedding_B") and name in getattr(m, "lora_embedding_B", {}):
                        torch.nn.init.normal_(m.lora_embedding_B[name], mean=0.0, std=0.02, generator=gen)
