...
class AdapterModel(nn.Module):
    ...
    # ===== Debug helpers: paste INTO AdapterModel class =====
    def _is_adapter_param_name(self, name: str) -> bool:
        return (".lora_" in name) or (".lora_embedding_" in name)

    def _extract_adapter_from_name(self, name: str):
        # name 예) "...q_proj.lora_A.default.weight"
        #        "...q_proj.lora_B.Adapter_3.weight"
        #        "...embed.lora_embedding_A.default.weight"
        tags = ("lora_A.", "lora_B.", "lora_embedding_A.", "lora_embedding_B.")
        for t in tags:
            if t in name:
                after = name.split(t, 1)[1]
                return after.split(".", 1)[0]
        return None

    def debug_adapters_placement(self, active: str = None, max_show_per_adapter: int = 2):
        """
        어댑터별 파라미터가 어느 디바이스에 있는지 요약 출력.
        - active 미지정 시 self.model.active_adapter 사용
        - 샤딩 환경에서도 device.type 만 보면 충분 (cuda:0, cuda:1 모두 'cuda')
        """
        active = active or getattr(self.model, "active_adapter", None) or "default"
        summaries = {}  # {adapter: {device_str: count}}
        samples = {}    # {adapter: [(name, device_str), ...]}

        for name, p in self.model.named_parameters():
            if not self._is_adapter_param_name(name):
                continue
            an = self._extract_adapter_from_name(name) or "unknown"
            d = p.device
            dstr = f"{d.type}:{d.index}" if d.type == "cuda" else d.type

            summaries.setdefault(an, {}).setdefault(dstr, 0)
            summaries[an][dstr] += 1

            if summaries[an][dstr] <= max_show_per_adapter:
                samples.setdefault(an, []).append((name, dstr))

        print("=" * 60)
        print(f"[Adapter placement] active='{active}'")
        for an in sorted(summaries.keys()):
            counts = ", ".join([f"{dev}={cnt}" for dev, cnt in sorted(summaries[an].items())])
            print(f"  - {an:>12}: {counts}")
            # 샘플 몇 줄
            for (n, dstr) in samples.get(an, [])[:max_show_per_adapter]:
                print(f"      • {n} -> {dstr}")
        print("=" * 60)

    def assert_adapters_placement(self, active: str = None, strict_cpu_for_inactive: bool = True):
        """
        활성 어댑터는 전부 CUDA(샤딩이면 cuda:0/1 등), 비활성은 CPU(옵션)인지 검사.
        - strict_cpu_for_inactive=False 로 주면 비활성의 device가 CPU '만' 요구하지 않고,
        그냥 '활성만 CUDA인지'를 확인(좀 더 느슨한 체크)합니다.
        """
        active = active or getattr(self.model, "active_adapter", None) or "default"
        ok = True
        bad = []
        ...
        for name, p in self.model.named_parameters():
            if not self._is_adapter_param_name(name):
                continue
            an = self._extract_adapter_from_name(name) or "unknown"
            d = p.device.type  # 'cuda' or 'cpu'

            if an == active:
                if d != "cuda":
                    ok = False
                    bad.append((name, d, "ACTIVE_SHOULD_BE_CUDA"))
            else:
                if strict_cpu_for_inactive and d != "cpu":
                    ok = False
                    bad.append((name, d, "INACTIVE_SHOULD_BE_CPU"))

        if ok:
            print(f"[OK] Adapter placement valid (active='{active}')")
        else:
            print(f"[MISMATCH] Adapter placement issues (active='{active}'):")
            for n, d, why in bad[:20]:
                print(f"  - {why}: {n} -> {d}")
            if len(bad) > 20:
