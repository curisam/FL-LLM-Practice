...
def get_llm(config, load_from_prev_ckpt=False, **kwargs):
    ...
    for m in model._iter_lora_modules():
        ...
        if name_a in m.lora_A and name_b in m.lora_A:
            a = m.lora_A[name_a]
            b = m.lora_A[name_b]
            ...
            # same storage (shared memory) 확인
            try:
                same_storage = a.storage().data_ptr() == b.storage().data_ptr()
            except Exception:
                same_storage = False
            print("same storage?", same_storage)
            print("values equal?", torch.equal(a_cpu, b_cpu))
            print("max diff:", float((a_cpu - b_cpu).abs().max().item()))
            print("a device/dtype:", a.device, a.dtype, "b device/dtype:", b.device, b.dtype)
