...
class AdapterModel(nn.Module):
    ...
    def append_adapters_random(self, adapter_names, peft_config=None):
        assert isinstance(self.model, PeftModel)
        peft_config = self.peft_config if peft_config is None else peft_config

        # 기본 시드(각 어댑터마다 다른 시드를 사용해서 초기화)
        base_seed = int(torch.randint(0, 2**31 - 1, (1,)).item())

        def _collect_tensors(obj):
            """Return a list of torch tensors/parameters to initialize from obj.

            obj can be a Tensor/Parameter, or an nn.Module (in which case we try
            to prefer .weight or top-level non-recursive parameters).
            """
            tensors = []
            if isinstance(obj, (torch.nn.Parameter, torch.Tensor)):
                tensors = [obj]
                return tensors

            if isinstance(obj, nn.Module):
                # prefer explicit weight attribute
                w = getattr(obj, "weight", None)
                if isinstance(w, (torch.nn.Parameter, torch.Tensor)):
                    tensors = [w]
                    return tensors

                # try top-level parameters (non-recursive)
                for _, p in obj.named_parameters(recurse=False):
                    tensors.append(p)
                if tensors:
                    return tensors
            ...
            # anything else: try .data if exists and tensor-like
        ...
        def _init_normal_safe(obj, mean, std, gen, fallback_seed):
            """Initialize tensor(s) represented by obj with N(mean,std).

            Tries to use torch.nn.init.normal_(..., generator=gen). If that
            raises TypeError (old PyTorch), falls back to temporarily seeding
            global RNGs using fallback_seed so initialization is reproducible.
            """
            tensors = _collect_tensors(obj)
            if not tensors:
                return

            # try generator-aware init first
            try:
                for t in tensors:
                    torch.nn.init.normal_(t, mean=mean, std=std, generator=gen)
                return
            except TypeError:
                # generator not supported; fallback to temporarily seeding globals
        ...
        for i, name in enumerate(adapter_names):
            ...
            # 어댑터별로 LoRA 파라미터를 별도 시드로 재초기화
            gen = torch.Generator()
            gen.manual_seed(base_seed + i)
            with torch.no_grad():
                for m in self._iter_lora_modules():
                    # LoRA A/B 텐서가 존재하면 새 난수로 덮어씀
                    if hasattr(m, "lora_A") and name in getattr(m, "lora_A", {}):
                        _init_normal_safe(getattr(m, "lora_A")[name], mean=0.0, std=0.02, gen=gen, fallback_seed=base_seed + i)
                    if hasattr(m, "lora_B") and name in getattr(m, "lora_B", {}):
                        _init_normal_safe(getattr(m, "lora_B")[name], mean=0.0, std=0.02, gen=gen, fallback_seed=base_seed + i)
                    # 임베딩 타입도 있으면 처리
                    if hasattr(m, "lora_embedding_A") and name in getattr(m, "lora_embedding_A", {}):
                        _init_normal_safe(getattr(m, "lora_embedding_A")[name], mean=0.0, std=0.02, gen=gen, fallback_seed=base_seed + i)
                    if hasattr(m, "lora_embedding_B") and name in getattr(m, "lora_embedding_B", {}):
                        _init_normal_safe(getattr(m, "lora_embedding_B")[name], mean=0.0, std=0.02, gen=gen, fallback_seed=base_seed + i)
    ...
    def _list_adapter_names_safe(self):
        ...
        """
        1) self.adapter_names 있으면 그걸 신뢰
        2) peft_config 키(dict)에서 이름 수집
        3) 모든 LoRA 모듈을 뒤져서 m.lora_A / m.lora_embedding_A 키(=어댑터명) 수집
        4) 그래도 비면 active_adapter 혹은 'default'로 최소 하나 채움   
    
        """

        # Start with any names we've explicitly tracked on this AdapterModel
        names = set()
        if hasattr(self, "adapter_names") and len(getattr(self, "adapter_names", [])) > 0:
            names.update(list(self.adapter_names))

        # Merge names from PEFT config if present
        ...
        # Also inspect LoRA modules for registered adapter keys
        for m in self._iter_lora_modules(): # LoRA가 달린 모듈만 추립니다
            if hasattr(m, "lora_A"):
                try:
        ...
        # If still empty, fall back to active adapter or 'default'
        if not names:
    ...
    def _move_adapter_on_module(self, m: nn.Module, adapter: str, device: torch.device): #결과: GPU에는 필요한 어댑터만 남고, 나머지는 CPU로 내려가 GPU 메모리를 즉시 절약합니다.
