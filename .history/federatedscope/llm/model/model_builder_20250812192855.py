...
def get_llm(config, load_from_prev_ckpt=False, **kwargs):
    ...
    if config.llm.adapter.use and config.llm.adapter.local_only: # config.llm.adapter.local_only=False로 걸림
        ...
        #각 이름에 대해 **동일한 LoRA 설정(r=8, alpha=16, dropout=0.05, target_modules=...)**으로 추가 어댑터를 등록해둠.
        #하지만 forward에서는 활성 어댑터의 A/B만 쓰여서 W x + (α/r)·B_active(A_active x) 형태로 계산됩니다. 나머지 어댑터는 “붙어만 있고” 적용되지 않아요.
