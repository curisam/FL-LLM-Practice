...
class AdapterModel(nn.Module):
    ...
    def load_state_dict(self, state_dict, strict=False):
        return self.model.load_state_dict(state_dict, strict=False) #전달받은 state_dict를 부분 로딩(missing/unexpected 허용)으로 주입. self.model은 PEFT 클래스라 .state_dict로 얻어진 것 바탕으로 모델이 업로드 잘 됨.
