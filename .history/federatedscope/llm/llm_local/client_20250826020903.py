...
class LLMMultiLoRAClient(Client):
    ...
    def _register_default_handlers(self):
        ...
        self.register_handlers('adapter_eval',
                               ...
                               ['grouping']) #서버로부터 'adapter_eval' 이름의 Message를 받음. content는 해당 클라이언트가 어떤 adpater를 사용해야하는지에 대한 index 정보.
        
        self.register_handlers('set_active_adapter_idx',
                               self.callback_funcs_for_setting_adapter_idx,
                               [None])#서버로부터 'set_active_adapter_idx' 이름의 Message를 받음. content는 해당 클라이언트가 어떤 adpater를 사용해야하는지에 대한 index 정보.
    ...
    def callback_funcs_for_model_para(self, message: Message):
        ...
        mm = acc.unwrap_model(m) if acc is not None else (m.module if hasattr(m, "module") else m)
        adapter_idx = None

        #학습 시작 이전에 adapter 할당

        if getattr(self._cfg.llm.adapter, "local_only", False):
            # 고정 어댑터 (없으면 0)
            ...
            if self._cfg.llm.adapter.warmup.use and self.state < total_warmup_round: #워밍업 라운드: adapter_idx = state // warmup_round로 0..(count-1) 순환 활성화.
                ...
                with torch.no_grad():
                    ...
                    for i in range(self._cfg.llm.adapter.count):
                        mm.set_active_adapter(f'Adapter_{i}')
                        m.eval()
            ...
            raise ValueError("Invalid adapter config: set llm.adapter.local_only=True or llm.adapter.count>1.")
            ...
            m.set_active_adapter(f'Adapter_{adapter_idx}')
            m.train()
        else:
            raise ValueError(
    ...
    def callback_funcs_for_adapter_eval(self, message: Message): #클라이언트가 모든 adapter의 성능을 validation data로 측정해서 서버로 전달.
        ...
        metrics = {}
        with torch.no_grad():
            # 모델 핸들: trainer.model을 기준으로
            m = self.trainer.model
            acc = getattr(self.trainer, "accelerator", None)

            # DDP/Accelerate 래핑 대비: set_active_adapter가 래퍼에 없다면 .module로 접근
            if acc is not None:
                mm = acc.unwrap_model(m)
            else:
                mm = m if hasattr(m, "set_active_adapter") else getattr(m, "module", m)
            ...
            for i in range(self._cfg.llm.adapter.count):
                ...
                adap_metrics = self.trainer.evaluate(target_data_split_name='val')
                logger.info(f'Client {self.ID} Adapter {i} -> {adap_metrics}')

                loss_i = adap_metrics.get('val_avg_loss')


                metrics[f'adapter_{i}_avg_loss'] = loss_i

        # 서버로 전송 (서버는 이 결과로 grouping/클러스터링 후 adapter_idx를 할당)
        if self._is_main_process():
            self.comm_manager.send(
