    ...
    def callback_funcs_for_adapter_eval(self, message: Message) -> bool:
        ...
        with torch.no_grad():
            for i in range(self._cfg.llm.adapter.count):
                ...
                # 활성화된 어댑터 관련 파라미터만 출력
                for name, param in self.model.named_parameters():
                    if active_adapter_name in name:  # 어댑터 이름이 포함된 파라미터만 출력
                        print(f'Active Adapter - Name: {name}, Shape: {param.shape}')


                # 어댑터 전환 후 파라미터 확인
                for name, param in self.model.named_parameters():
                    if "adapter" in name:
                        logger.info(f'Adapter {i} - {name}: {param.data[:5]}')  # 앞 5개 값 확인
