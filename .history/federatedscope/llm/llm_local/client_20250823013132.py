...
class LLMMultiLoRAClient(Client):
    ...
    def callback_funcs_for_model_para(self, message: Message):
        ...
        if getattr(self._cfg.llm.adapter, "local_only", False):
            ...
            logger.info(f"Activate fixed adapter {adapter_idx} for training...")
    ...
    def callback_funcs_for_adapter_eval(self, message: Message): #클라이언트가 각 adapter의 성능을 validation data로 측정해서 서버로 전달.
        ...
        with torch.no_grad():
            ...
            for i in range(self._cfg.llm.adapter.count):
                ...
                if loss_i is None:
                    # val이 없거나 키가 없으면 임시값/표식 (원래 로직 유지하려면 random.random() 가능)
                    # 여기선 명시적으로 None을 넣어 서버가 후처리하기 쉽게 함
                    metrics[f'adapter_{i}_avg_loss'] = None
                else:
