...
class LLMMultiLoRAServer(Server):
    ...
    def __init__(self,
        ...
        if self._cfg.llm.adapter.grouping.use: #True
            self.msg_buffer['adapter_eval'] = dict()

    def _register_default_handlers(self): #client: 'grouping'  msg_type 보냄 -> Server: 'set_active_adapter_idx'로 처리 (set_active_adapter_idx)
    ...
    # def _perform_federated_aggregation(self):
    #     """
    #     LoRA 어댑터가 여러 개인 상황에 맞춘 메모리 절약형 집계
    #     ----------------------------------------------------
    #     * state_dict 복사본을 만들지 않고 in-place 로 파라미터 갱신
    #     * staleness 계산(지연 클라이언트)은 LoRA 모드에서 사용하지 않으므로 제외
    #     """
    #     train_msg_buffer = self.msg_buffer['train'][self.state]
    #     aggregated_num = 0
    ...
    #         # 1) 이번 라운드에 들어온 모든 (data_size, adapter_state_dict) 모으기
    #         msg_list = []
    #         for client_id, content in train_msg_buffer.items():
    #             if self.model_num == 1:
    #                 msg_list.append(content)                        # (n, state_dict)
    #             else:
    ...
    #         warmup_round        = self._cfg.llm.adapter.warmup.round
    #         total_warmup_round  = warmup_round * self._cfg.llm.adapter.count
    #         in_warmup_phase     = (self._cfg.llm.adapter.warmup.use and
    #                                self.state < total_warmup_round)

    #         if in_warmup_phase:
    #             result_state = aggregator.aggregate(agg_info)
    #         else:
    #             result_state = aggregator.aggregate_on_model(agg_info)

    #         # 3) 새로운 state_dict → 기존 모델 파라미터로 in-place copy
    #         with torch.no_grad():
