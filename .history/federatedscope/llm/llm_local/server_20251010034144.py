...
class LLMMultiLoRAServer(Server):
    ...
    #     adapter_idx -> [client_ids] 고정 그룹핑을 구성.
    #     """
    #     clusters_1b = None
    #     rt = getattr(self._cfg.llm.adapter, 'cluster_runtime', None)
    #     if rt and 'schedule_file' in rt:
    #         import json, os
    #         p = rt['schedule_file']
    #         if os.path.exists(p):
    #             with open(p, 'r') as f:
    #                 info = json.load(f)
    #             clusters_1b = info.get('clusters_1_based')

    #     if clusters_1b is None:
    #         clusters_1b = getattr(self._cfg.llm.adapter, 'clusters', None)  # 1-based 전제

    #     if clusters_1b is None:
    #         raise ValueError("cluster 고정 그룹핑 초기화 실패: 클러스터 정보를 찾을 수 없음")

    #     # 1-based ID를 유지
    #     self._fixed_groups = {i: sorted(g) for i, g in enumerate(clusters_1b)}
    #     self._client2adp = {cid: i for i, g in self._fixed_groups.items() for cid in g}
    ...
    def trigger_for_start(self):
        ...
        if self.check_client_join_in(): ##전체 클라이언트 수만큼 join_in이 반영됐는지 여부
            ...
            if self._grouping_is_fixed:
                ...
                s_cur = self._current_required_sample_num()   # Server의 도우미 메서드 상속됨
