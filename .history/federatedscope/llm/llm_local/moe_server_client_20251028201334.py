...
class FullMoEClient(_BaseMoEClient):
    ...
    def callback_funcs_for_model_para(self, message: Message) -> bool:
        ...
        round_ = message.state
        sender = message.sender
        timestamp = message.timestamp
        content = message.content

        if self._cfg.federate.process_num > 1 and content is not None:
        ...
        # ── 3) 로컬 학습(Full‑MoE 트레이너가 어댑터별 가중합 손실로 학습) ──
        sample_size, model_para_all, _ = self.trainer.train(round_num=round_)
