...
class LLMMultiLoRAClient(Client):
    ...
    def callback_funcs_for_model_para(self, message: Message):
        ...
        if self._cfg.llm.adapter.count > 1: #3. “군집/선택” 모드:
            ...
            total_warmup_round = \
                warmup_round * self._cfg.llm.adapter.count #75
            if self._cfg.llm.adapter.warmup.use and \
                    self.state < total_warmup_round: #워밍업 라운드: adapter_idx = state // warmup_round로 0..(count-1) 순환 활성화.
                ...
                with torch.no_grad():
                    ...
                    for i in range(self._cfg.llm.adapter.count):
                        ...
                        metrics = self.trainer.evaluate(
                            target_data_split_name='val')#global model에서 validation 기준 결과들.
                        logger.info(
                            f'Adapter {i} with the results: {metrics}')
                        if i == 0 or min_loss > metrics['val_avg_loss']: #i==0은 초기값으로 임시 지정. 그 이후에는 가장 min loss로 나오는 것으로 업데이트
                            min_loss, adapter_indices = metrics[
                                'val_avg_loss'], [i]
                        elif min_loss == metrics['val_avg_loss']: #동률일 경우에는 추가.
                            adapter_indices.append(i)
        ...
        train_data_size = len(self.data.train_data)#dataset size.
