...
class LLMMultiLoRAServer(Server):
    ...
    """
    그루핑 주기(예: r 라운드마다) + 워름업 끝난 시점에 도달하면

    클라에게 adapter_eval을 브로드캐스트해 각 LoRA 어댑터의 평가(avg_loss 등) 를 모든 클라가 수행하도록 시킴

    그 결과(클라→서버)가 다 모이면(아래 check_and_grouping) 그때 그루핑을 실행하고,
    ...
    def _start_new_training_round(self, aggregated_num=0, skip_grouping=False):
        if self._cfg.llm.adapter.grouping.use and not skip_grouping:
            ...
            if self._cfg.llm.adapter.warmup.use: #warm up round 갯수를 총 lora adapter 갯수배 만큼 증강.
