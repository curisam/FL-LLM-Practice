...
assert torch.__version__.startswith("2.0.0"), f"[guard] unexpected torch {torch.__version__}"
assert torch.version.cuda == "11.7", f"[guard] unexpected torch CUDA build {torch.version.cuda}"
# vision/audio는 나중에 lazy import되는 경우가 있으니, 메타데이터와 모듈 버전 둘 다 확인
try:
    import torchvision, torchaudio
except Exception:
    # 아직 import 안 했어도 배포 메타데이터는 있어야 함
    pass
else:
    assert torchvision.__version__.startswith("0.15.0"), f"[guard] unexpected torchvision {torchvision.__version__}"
    assert torchaudio.__version__.startswith("2.0.0"), f"[guard] unexpected torchaudio {torchaudio.__version__}"

# 배포 메타데이터(dist-info)도 torch 2.0.0인지 확인(실행 중 재설치 등 레이스 방지)
assert _im.version("torch").startswith("2.0.0"), f"[guard] metadata torch {_im.version('torch')}"
# ===================================
