...
class Monitor(object):
    ...
    def format_eval_res(self,
        ...
        #####  callback_funcs_for_evaluate 혹은 유사한 evaluate 핸들러 내에서 monitor.format_eval_res(results_client, rnd, role=f"Client #{cid}", forms=['raw'], return_raw=True)→ Raw 라인(JSONL)으로 기록하는 용도
        # 서버: federatedscope/core/workers/server.py (avg/weighted_avg/fairness 집계 만들 떄)
        #####  formatted = monitor.format_eval_res(results_aggregated, rnd, role="Server #", forms=['weighted_avg','avg','fairness','raw'])
            ...
            for form in forms:
                ...
                if not str(role).lower().startswith('server') or form == 'raw': #클라이언트 역할이거나 raw 포맷이면: 그냥 원본(results)을 Results_raw로 붙이고 끝.
                    ...
                    for key in results.keys():
                        ...
                        if form == 'weighted_avg': #임의의 key에 대해서 weight avg. train/val/test인지는 앞선 dataset_name에서 결정됨. 
