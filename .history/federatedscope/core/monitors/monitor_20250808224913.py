...
class Monitor(object):
    ...
    def eval(self, ctx): #eval_metric의 key들에 있는 것에 대해 측정한 값들 바탕으로 dictionary return.
        """
    ...
    def global_converged(self): #🔹 목적: 학습 도중 글로벌 수렴(Global convergence)이 이루어진 시점을 기록
    ...
    def local_converged(self): # 🔹 목적: 클라이언트 로컬 수렴 시점 기록 (서버와는 별개)
    ...
    def finish_fl(self): #🔹 목적: 학습 종료 시, 시스템 지표들을 system_metrics.log 파일에 기록 (단, rank 0에서만 실행)
        """
    ...
    def format_eval_res(self,
                        ...
                        role=-1,
                        forms=None,
                        return_raw=False): #🔹 목적: 각 라운드 결과(results)를 여러 형태(avg, weighted_avg, fairness, raw)로 포맷하여 반환
        """
        ...
        'weighted_avg': 데이터 수 기반 가중 평균

        'avg': 단순 평균

        'fairness': 클라이언트 간 분산/하위 decile 등

        'raw': 클라이언트별 결과 그대로

        📦 내부 로직:

        client_1, client_2, ... 들의 metric 값을 모아서 연산

        raw 외 결과는 server 기준일 때만 생성
    ...
    def calc_model_metric(self, last_model, local_updated_models, rnd): #🔹 목적: 모델 수준의 메트릭(cos_sim, l2_distance, 등)을 계산하여 로깅
        """
        Arguments:
    ...
    def track_avg_flops(self, flops, sample_num=1): #🔹 목적: 샘플당 평균 FLOPs를 추적. 누적 방식: moving average
        """
    ...
    def track_upload_bytes(self, bytes): #목적: 클라이언트가 서버로 업로드한 모델 크기 누적. 예시: StandaloneDDPCommManager.send() 내부에서 호출
        """
        Track the number of bytes uploaded.
        """
        self.total_upload_bytes += bytes

    def track_download_bytes(self, bytes): #목적: 서버가 클라이언트로 내려보낸 모델 크기 누적
        """
        Track the number of bytes downloaded.
        """
        self.total_download_bytes += bytes

    def update_best_result(self, best_results, new_results, results_type): #🔹 목적: 현재 평가 결과가 기존 best보다 더 낫다면 갱신. 📦 로직 요약: round_wise_update_key에 따라 더 좋은 성능인지 판단 (loss는 작을수록, acc는 클수록)
