...
def _is_main_process():
    """accelerate > torch.distributed > ENV(RANK) 순으로 main 여부 판단"""
    # accelerate
    try:
        from accelerate.state import AcceleratorState
        st = AcceleratorState()
        if st.num_processes > 1:
            return st.process_index == 0
    except Exception:
        pass
    # torch.distributed
    try:
...
class Monitor(object):
    ...
    def __init__(self, cfg, monitored_object=None):
        ...
        # [변경] outdir 정규화로 sub_exp 제거 → 단일 폴더 고정
        self.outdir = _normalize_outdir(cfg.outdir)
