...
def _install_rank0_file_logger_once(outdir: str):
    ...
    # 이미 exp_print.log 핸들러가 달려있으면 스킵
    for h in list(root.handlers):
        if isinstance(h, logging.FileHandler):
            try:
                if os.path.basename(getattr(h, 'baseFilename', '')) == "exp_print.log":
...
class Monitor(object):
    ...
    def __init__(self, cfg, monitored_object=None):
        ...
        #self.metric_calculator.eval_metric-> loss,acc, avg_loss, total, acc를 key로 갖는 dict -> #{'acc': (<function eval_acc at 0x7f2edf2eb310>, True), 'avg_loss': (<function eval_avg_loss at 0x7f2edf2eb790>, False), 'loss': (<function eval_loss at 0x7f2edf2eb700>, False), 'total': (<function eval_total at 0x7f2edf2eb820>, False)}
        ...
        self.global_convergence_round = 0
        self.global_convergence_wall_time = 0
        self.local_convergence_round = 0
