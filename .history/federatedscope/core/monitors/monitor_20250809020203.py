...
class Monitor(object):
    ...
    def format_eval_res(self,
                        ...
                        role=-1,
                        forms=None,
                        return_raw=False): #🔹 목적: 각 라운드 결과(results)를 여러 형태(avg, weighted_avg, fairness, raw)로 포맷하여 반환 #평가 로그 상황에서 호출
        """
    ...
    def calc_model_metric(self, last_model, local_updated_models, rnd): #🔹 목적: 모델 수준의 메트릭(cos_sim, l2_distance, 등)을 계산하여 로깅 #서버 라운드 끝에서 호출
        """
        Arguments:
