...
class Monitor(object):
    ...
    def __init__(self, cfg, monitored_object=None):
        ...
        self.metric_calculator = MetricCalculator(cfg.eval.metrics) #loss,acc, avg_loss, total -> {'acc': (eval_acc, True), 'loss': (eval_loss, False) ....}
        ...
        self.the_larger_the_better = self.metric_calculator.eval_metric[
            update_key][1]#False로 나옴. loss는 크면 안좋은거기때문에
    ...
    def local_converged(self):
        ...
        ) - self.fl_begin_wall_time #FL 과정 전체가 시작된 시점(self.fl_begin_wall_time)으로부터 로컬 조기 종료가 결정되기까지 경과한 실제 벽시계 시간(wall-clock time)을 계산해서 저장
    ...
    def merge_system_metrics_simulation_mode(self,
        ...
        if self.use_wandb and self.wandb_online_track: #해당 안함.
            try:
                import wandb
    ...
    def finish_fed_runner(self, fl_mode=None):
        ...
        if self.use_wandb and not self.wandb_online_track: #해당 안함.
            try:
                import wandb
            except ImportError:
                logger.error(
    ...
    def format_eval_res(self,
        ...
        # ✅ rank0만 raw 결과 파일에 append (다중 파일 생성 차단)
    ...
    def track_model_size(self, models): # #모델 전체 사이즈 측정.
        """
    ...
    def track_avg_flops(self, flops, sample_num=1): #flop 관한 것 측정. 실제로 안 다루므로 PASS
        """
