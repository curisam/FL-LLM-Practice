...
class Monitor(object):
    ...
    def format_eval_res(self,
        ...
        #####  클라 결과 수집 후 집계 단계에서 monitor.format_eval_res(…, forms=['weighted_avg','avg','fairness','raw'])→ monitor.save_formatted_results(…)-> eval_results.log에 기록
        """
        ...
        if 'group_avg' in forms: #특수 분기
            ...
            for form in forms:
                ...
                if not str(role).lower().startswith('server') or form == 'raw': #클라이언트 역할이거나 raw 포맷이면: 그냥 원본(results)을 Results_raw로 붙이고 끝.
                    ...
                    continue
                else: #서버 역할 + SUPPORTED_FORMS(= ['weighted_avg','avg','fairness','raw'])일 때만 아래 집계 계산을 수행.
