...
class Monitor(object):
    ...
    def format_eval_res(self,
                        ...
                        role=-1,
                        forms=None,
                        return_raw=False): #🔹 목적: 각 라운드 결과(results)를 여러 형태(avg, weighted_avg, fairness, raw)로 포맷하여 반환 #평가 로그 상황에서 서버에서 집계목적으로 호출
        #format_eval_res(results, rnd=R, role='Server #', forms=['weighted_avg','avg','fairness','raw'])
        """
