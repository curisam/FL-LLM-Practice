...
class MetricCalculator(object):
    ...
    def __init__(self, eval_metric: Union[Set[str], List[str], str]):
        ...
        self.eval_metric = self.get_metric_funcs(eval_metric)# return 형태: {'acc': (eval_acc, True), 'loss': (eval_loss, False), ...}. acc, loss, avg_loss, total을 key로 가짐. total은  총 샘플 수. 
    ...
    def get_metric_funcs(self, eval_metric):
        ...
        metric_buildin = {
            ...
            for metric in {'loss', 'avg_loss', 'total'} | eval_metric  #union of {'loss', 'avg_loss', 'total'} and  eval_metric
            if metric in SUPPORT_METRICS
        }
        import ipdb; ipdb.set_trace(context=15)
        metric_register = get_metric(eval_metric - set(SUPPORT_METRICS.keys()))#차집합에 대해서 dictionary 얻는다. 차집합이 공집합이라 {}.
        return {**metric_buildin, **metric_register} # return 형태: {'acc': (eval_acc, True), 'loss': (eval_loss, False), ...} acc, loss, avg_loss, total을 받는다.
    #{'acc': (<function eval_acc at 0x7f2edf2eb310>, True), 'avg_loss': (<function eval_avg_loss at 0x7f2edf2eb790>, False), 'loss': (<function eval_loss at 0x7f2edf2eb700>, False), 'total': (<function eval_total at 0x7f2edf2eb820>, False)}

    def eval(self, ctx):
        #Trainer 내부 ctx에서 결과(예: ys_true, ys_pred, ys_prob)를 받아 메트릭 계산
        #_check_and_parse(ctx)를 통해 y_true, y_pred, y_prob를 추출
        #설정된 메트릭들 각각에 대해 계산 수행
        results = {}
    ...
    def _check_and_parse(self, ctx): #y_true, y_pred, y_prob를 반환
        """
...
"""{
  'hits@1': (eval_hits, True),
  'hits@2': (eval_hits, True),
  ...
  'hits@100': (eval_hits, True)
}"""
