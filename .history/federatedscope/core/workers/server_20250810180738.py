...
class Server(BaseServer): #클라이언트 등록, 메시지 수신 및 응답 처리, 모델 평가용 사본 생성 등 실제 FL 서버 기능 구현
        ...
        if self.check_buffer(self.state, min_received_num, check_eval_result): #min_recieved_num만큼 버퍼가 찬 상황
            if not check_eval_result: #Train 상황
                ...
                if self.state % self._cfg.eval.freq == 0 and self.state != \
                    ...
                    self.eval() #server 모델을 모든 클라이언트들한테 보냄
    ...
    def broadcast_model_para(self,
                             msg_type='model_para',
                             sample_client_num=-1,
                             filter_unseen_clients=True): #서버가 클라이언트들에게 중앙 서버 모델을 보내주는 핵심 함수
        """
        ...
        """
        if filter_unseen_clients: #FALSE
    ...
    def broadcast_client_address(self): #클라이언트 모두에게 참여하는 모든 클라이언트 정보들을 보냄.
        """
