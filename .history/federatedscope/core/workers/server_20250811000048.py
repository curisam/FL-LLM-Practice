        ...
        for merge_type, eval_res_set in [("participated",
            ...
            if eval_res_set != []:
                ...
                self._monitor.update_best_result(
                    ...
                    if merge_type == "unseen" else "client_best_individual") #result_type= "client_best_individual". 모든 클라이언트들의 평균이 아닌 가장 낮게 나오는 클라이언트 값 기준으로 best(test_loss) 갱신. 평균으로 해야 맞지 않나??

                self._monitor.save_formatted_results(formatted_logs) #formatted_logs를 eval_results.log에 한줄 추가.
