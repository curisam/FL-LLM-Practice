...
class Client(BaseClient):
    ...
    def callback_funcs_for_model_para(self, message: Message):
        ...
        """
        _wrap_comm_send_debug(self.comm_manager)  # ⬅️ 맨 위쪽 어딘가 1회만
        ...
        # self.state = round  바로 뒤에:
        _dump_queue_snapshot(self.comm_manager, tag=f"MODELPARA:entry r={round}", drain=False, filter_state=round)
        ...
        _dump_queue_snapshot(self.comm_manager, tag=f"MODELPARA:after-train r={self.state}", drain=False, filter_state=self.state)        
        ...
        print("############################ Before ############################")
        print(len(self.comm_manager.comm_queue))
        ...
        _dump_queue_snapshot(self.comm_manager, tag=f"MODELPARA:after-send r={self.state}", drain=False, filter_state=self.state)
    ...
    def callback_funcs_for_evaluate(self, message: Message):
        ...
        _dump_queue_snapshot(self.comm_manager, tag=f"EVAL:entry r={message.state}", drain=False, filter_state=message.state)
        ...
        try:
            ...
            for split in self._cfg.eval.split:  #['val', 'test']
                ...
                _dump_queue_snapshot(self.comm_manager, tag=f"EVAL:after-{split} r={self.state}", drain=False, filter_state=self.state)
        ...
        if is_main:
            _dump_queue_snapshot(self.comm_manager, tag=f"EVAL:after-monitor r={self.state}", drain=False, filter_state=self.state)
        ...
        print("############################ Before ############################")
        print(len(self.comm_manager.comm_queue))
        _dump_queue_snapshot(self.comm_manager, tag=f"EVAL:before-send r={self.state}", drain=False, filter_state=self.state)
        ...
        print("############################ After ############################")
        print(len(self.comm_manager.comm_queue))
        _dump_queue_snapshot(self.comm_manager, tag=f"EVAL:after-send r={self.state}", drain=False, filter_state=self.state)
