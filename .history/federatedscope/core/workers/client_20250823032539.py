...
class Client(BaseClient):
    ...
    # def _is_main_process(self) -> bool:
    #     # 1) accelerator 살아 있으면 그 값 사용
    #     trainer = getattr(self, 'trainer', None)
    #     if trainer is not None and hasattr(trainer, 'accelerator') and trainer.accelerator is not None:
    #         return bool(trainer.accelerator.is_main_process)

    #     # 2) 환경변수 기반 판별 (accelerate/torchrun/slurm 등 공통)
    #     import os
    #     for k in ("RANK", "PMI_RANK", "SLURM_PROCID"):
    #         v = os.environ.get(k)
    #         if v is not None:
    #             try:
    #                 return int(v) == 0  # rank==0만 True
    #             except ValueError:
    #                 pass

    #     # 3) 환경변수도 없으면 싱글 프로세스 가정
    #     return True

    def _rank_world(self):
        import os
        # 1) 환경변수 먼저 (accelerate/torchrun/slurm 공통)
        rank_env  = os.environ.get("RANK") or os.environ.get("PMI_RANK") or os.environ.get("SLURM_PROCID")
        world_env = os.environ.get("WORLD_SIZE") or os.environ.get("PMI_SIZE") or os.environ.get("SLURM_NTASKS")

        rank  = int(rank_env)  if rank_env  is not None else None
        world = int(world_env) if world_env is not None else None

        # 2) 환경변수 없으면 Accelerator에서 보조
        acc = getattr(self.trainer, "accelerator", None)
        if rank  is None and acc is not None:  rank  = int(getattr(acc, "process_index", 0))
        if world is None and acc is not None:  world = int(getattr(acc, "num_processes", 1))

        # 3) 그래도 없으면 단일 프로세스 가정
        if rank  is None: rank  = 0
        if world is None: world = 1
        return rank, world  
    ...
    def callback_funcs_for_evaluate(self, message: Message):
        ...
        try:
            ...
            for split in self._cfg.eval.split:  #['val', 'test']
                ...
                if self.trainer.ctx.eval_metrics:
