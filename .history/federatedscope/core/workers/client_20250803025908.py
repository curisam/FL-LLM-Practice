...
class Client(BaseClient):
    ...
    def _reset_ctx_split_metrics(self, split: str = 'train'):
        """
        trainer.ctx 안의 split(prefix) 관련 메트릭/버퍼를 라운드 시작 전에 초기화.
        예: split='train'이면 train_total, train_loss 등과 per-proc 버퍼를 모두 리셋.
        """
        ...
        try:
            # 1) 집계 메트릭(dict)에서 해당 split 키들 제거
            if hasattr(ctx, "eval_metrics") and isinstance(ctx.eval_metrics, dict):
                for k in list(ctx.eval_metrics.keys()):
                    if k.startswith(prefix):
                        del ctx.eval_metrics[k]

            # 2) rank별 로컬 스냅샷/버퍼 초기화
            if hasattr(ctx, "local_results_for_log"):
                lrf = getattr(ctx, "local_results_for_log")
                if isinstance(lrf, dict):
                    for k in list(lrf.keys()):
                        if k.startswith(prefix):
                            del lrf[k]

            # 3) per-proc 모음(리스트/딕셔너리 등) 초기화
            #    (코드베이스에 따라 None, [] 로 쓰이므로 안전하게 비움)
            if hasattr(ctx, "per_proc_local_results"):
                setattr(ctx, "per_proc_local_results", None)

            # 4) 혹시 커스텀 누적 카운터가 있다면 0으로
            for attr in ["accumulated_total", "accumulated_train_total"]:
                if hasattr(ctx, attr):
                    try:
                        setattr(ctx, attr, 0)
                    except Exception:
            ...
            self.logger.debug(f"[reset-metrics] failed to reset '{split}' metrics: {e}")
    ...
    def callback_funcs_for_model_para(self, message: Message): #서버로부터 받은 글로벌 모델 파라미터(혹은 시크릿 셰어 조각)를 처리하고, 로컬 학습을 트리거한 뒤 결과를 서버에 다시 보냅니다.
        ...
        """
        # === 동일 라운드의 학습 메시지는 1회만 처리 ===
        r = message.state
        if r in self._handled_rounds['model_para']:
            self.logger.debug(f"[skip duplicate train] round={r} already handled.")
            return
        self._handled_rounds['model_para'].add(r)
        # ----------------------------------------------
        ...
        if 'ss' in message.msg_type: #msg_type 에 'ss' 포함 ⇒ 비밀 분할 조각(fragment) 수신 처리. 해당 안함!!
            ...
            if self.is_unseen_client or skip_train_isolated_or_global_mode: #해당 안함
                ...
                # 라운드 시작 전, 이전 라운드의 train_* 잔여 메트릭/버퍼 제거
    ...
    def callback_funcs_for_evaluate(self, message: Message):
        ...
        """
        # === 동일 라운드의 평가 메시지도 1회만 처리 ===
        r = message.state
        if r in self._handled_rounds['evaluate']:
            self.logger.debug(f"[skip duplicate eval] round={r} already handled.")
            return
        self._handled_rounds['evaluate'].add(r)
        # ----------------------------------------------
