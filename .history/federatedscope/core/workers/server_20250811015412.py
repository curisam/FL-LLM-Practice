...
class Server(BaseServer): #클라이언트 등록, 메시지 수신 및 응답 처리, 모델 평가용 사본 생성 등 실제 FL 서버 기능 구현
    ...
    def __init__(self,
        ...
        self.data = data
        self.device = device
        self.best_results = dict() #"client_best_individual", "Results_weighted_avg", "Results_avg", "Results_fairness" 를 키로 가질 것임.
        self.history_results = dict()  #아래의 form을 eval 라운드 마다 누적해서 쌓을 것임
    ...
    def merge_eval_results_from_all_clients(self): # 같은 라운드에서 각 클라이언트가 보낸 평가 결과(metrics)를 모아 합치는 함수입니다.
        ...
        for merge_type, eval_res_set in [("participated",
            ...
            if eval_res_set != []:
                ...
                ##### self.best_results[f"Results_{metric_name}"] 업데이트 ##############  저장 여부는 weighted_avg 관점 test_loss 기준으로 결정하게 된다.
