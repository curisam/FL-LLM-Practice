...
class Client(BaseClient):
    ...
    def _append_raw_line(self, role_str: str, round_idx, results_dict: dict, filename: str):
        ...
        try:
            outdir = getattr(self._monitor, "outdir", None)
            logger.info(f"[DEBUG] about to write {filename} in outdir={outdir}")   # ← 이 줄 추가
            if not outdir:
    ...
    def callback_funcs_for_evaluate(self, message: Message):
        ...
        """
        logger.info(f"[DEBUG] → Enter callback_funcs_for_evaluate (Client #{self.ID}, round={message.state})")
        ...
        logger.debug(f"[DEBUG] is_main={is_main}, raw ctx.eval_metrics={raw_ctx}, merged metrics init={metrics}")
        ...
        try:
            if self.early_stopper.early_stopped and self._cfg.federate.method in ["local", "global"]:
                ...
                for split in self._cfg.eval.split:  # ['test', 'val'] 등
                    ...
                    eval_metrics = self.trainer.evaluate(target_data_split_name=split)
                    logger.debug(f"[DEBUG] after evaluate(split={split}): eval_metrics={eval_metrics}")
                    if eval_metrics:
        ...
        if is_main:
            ...
            logger.debug(f"[DEBUG] combined keys={list(combined.keys())}, has_val={has_val}, has_test={has_test}")
