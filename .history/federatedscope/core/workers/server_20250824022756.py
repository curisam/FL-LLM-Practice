...
class Server(BaseServer): #클라이언트 등록, 메시지 수신 및 응답 처리, 모델 평가용 사본 생성 등 실제 FL 서버 기능 구현
    ...
    def broadcast_model_para(self,
        ...
        if self._cfg.federate.share_local_model and not self._cfg.federate.online_aggr: #True
            model_para = {} if skip_broadcast else copy.deepcopy(self.models[0].state_dict()) #server model 적용 model_para = self.models[0].state_dict())
    ...
    def check_buffer(self,
        ...
        if check_eval_result:
            ...
            print(cur_buffer)
            print(len(cur_buffer))
            print(min_received_num)
