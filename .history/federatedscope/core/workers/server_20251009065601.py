...
class Server(BaseServer): #클라이언트 등록, 메시지 수신 및 응답 처리, 모델 평가용 사본 생성 등 실제 FL 서버 기능 구현
    ...
    def _current_required_sample_num(self, round_idx: int = None) -> int:
        ...
        if getattr(self._cfg.federate, 'sampler', 'uniform') != 'cluster':
            return int(self._cfg.federate.sample_client_num)
        boundaries = getattr(self._cfg.llm.adapter, 'boundaries', None)
        s_per = getattr(self._cfg.llm.adapter, 'sample_num_per_adapter', None)
        if not boundaries or not s_per:
            return int(self._cfg.federate.sample_client_num)
        aidx = bisect_right([int(b) for b in boundaries], int(round_idx))-1
    ...
    def broadcast_model_para(self,
        ...
        if sample_client_num > 0:
            ...
            # # cluster 모드: 이번 라운드의 어댑터 인덱스를 receiver에게 먼저 통지
            # if msg_type == 'model_para' and getattr(self._cfg.federate, 'sampler', 'uniform') == 'cluster':
            #     from bisect import bisect_right
            #     boundaries = getattr(self._cfg.llm.adapter, 'boundaries', [])
            #     aidx = bisect_right([int(b) for b in boundaries], int(self.state))  # 0..count-1
            #     self.comm_manager.send(Message(msg_type='set_active_adapter_idx',
            #                                 sender=self.ID, receiver=receiver,
            #                                 state=self.state, timestamp=self.cur_timestamp,
            #                                 content=aidx))
