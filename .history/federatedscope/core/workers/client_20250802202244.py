...
class Client(BaseClient):
    ...
    def _log_split_metrics(self, role_str, round_idx, split, trainer_ctx):
        ...
        trainer_ctx: 트레이너 컨텍스트 (self.trainer.ctx)
        """
    ...
    def callback_funcs_for_model_para(self, message: Message): #서버로부터 받은 글로벌 모델 파라미터(혹은 시크릿 셰어 조각)를 처리하고, 로컬 학습을 트리거한 뒤 결과를 서버에 다시 보냅니다.
        ...
        if 'ss' in message.msg_type: #msg_type 에 'ss' 포함 ⇒ 비밀 분할 조각(fragment) 수신 처리. 해당 안함!!
            ...
            if self.is_unseen_client or skip_train_isolated_or_global_mode: #해당 안함
                ...
                local_log = getattr(self.trainer.ctx, 'local_results_for_log', None)
                if local_log:
                    logger.info({
                        ...
                        'Rank':      f'{rank}/{world}',
                        'Local':     True,
                        'Results':   local_log,   # ← 120개 기준(각 rank의 로컬)
                    })

                # --- [집계 결과 출력(선택)] ---
                # 터미널에 집계본을 아예 안 찍고 싶으면 아래 블록을 주석 처리하세요.
                if (getattr(self.trainer, 'accelerator', None) is None) or self.trainer.accelerator.is_main_process:
                    train_log_res = self._monitor.format_eval_res(
                        results, rnd=self.state, role=f'Client #{self.ID}', return_raw=True)
            ...
            for split in self._cfg.eval.split: #['test', 'val'] 
                ...
                if local_log:
                    logger.info({
                        'Role':      f'Client #{self.ID}',
                        'Round':     self.state,
                        'Split':     split,                 # 'test' or 'val'
                        'Rank':      f'{rank}/{world}',
                        'Local':     True,
                        'Results':   local_log,             # test: 10개, val: 36~37개 기준
                    })
