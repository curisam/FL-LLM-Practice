...
class Server(BaseServer): #클라이언트 등록, 메시지 수신 및 응답 처리, 모델 평가용 사본 생성 등 실제 FL 서버 기능 구현
    ...
    def __init__(self,
        ...
        self.msg_buffer = {'train': dict(), 'eval': dict()}  #클라이언트로부터 수신한 메시지들을 라운드별로 저장
        
        """
        self.msg_buffer = {
            'train': defaultdict(list), # 거의 사용 안함
            'eval': defaultdict(list), # 거의 사용 안함
            'model_para': defaultdict(list), #클라이언트가 보낸 학습된 모델 파라미터 누적, 모델 집계
            'metrics': defaultdict(list) #클라이언트가 수행한 글로벌 평가 결과 저장 (서버가 요청했을 때), 성능 평가 및 early stop 판단
        }
        self.msg_buffer['model_para'][client_id]: 해당 client가 보낸 모델 파라미터 저장

        self.msg_buffer['metrics'][client_id]: 평가 결과 저장

        Aggregation 시점까지 메시지를 누적 관리하며, 수가 충분해지면 check_and_move_on() 수행
        """
    ...
    def _start_new_training_round(self, aggregated_num=0):
        ...
        if self._cfg.asyn.use:  # for asynchronous training #False
    ...
    def _merge_and_format_eval_results(self):
        ...
        #라운드별 서버 로그를 누적해서 나중에 early stop나 요약용으로 씀.
        ...
        for merge_type, eval_res_set in [("participated",
            ...
            if eval_res_set != []:
                ...
                formatted_logs = self._monitor.format_eval_res(
                    ...
                    rnd=round,
                    role='Server #',
                    forms=self._cfg.eval.report) #self._cfg.eval.report=[weighted_avg, avg, fairness, raw]  
                ...
                """
                self.best_results는 metrics_all_client와 같은 형태로  특정 기준 metric(예: test_loss) 기준 가장 낮은 값의 클라이언트에 해당하는 것이 더 낮아질 때마다 해당 시점의 값으로 갱신됩              
                                metrics_all_clients = {
                                    "val_acc":  [0.8, 0.75, 0.9],
                                    "val_loss": [0.5, 0.55, 0.45],
                                    "val_total": ~~
                ...
                ##### self.best_results["client_best_individual"] 업데이트 ############## 모든 클라이언트들의 평균이 아닌 가장 낮게 나오는 클라이언트 값 기준으로 best(test_loss) 갱신. 
                ...
                ##### self.best_results[f"Results_{metric_name}"] 업데이트 ############## 

                #update_prior_list로 정한 우선순위가 가장 높은 폼에서 베스트 갱신이 있었는지를 기준으로, 그 라운드에 체크포인트 저장 여부를 결정합니다.
                update_prior = -1  #지금까지 “대표로 채택한 form”의 우선순위 인덱스(작을수록 높음). 초기 -1은 아직 아무 것도 채택 안 함.
                    ...
                    if form != "raw":
                        ...
                        update_best_this_round_tmp = \
                            self._monitor.update_best_result(
                                ...
                                f"client_summarized_{form}") #self.best_results[f"client_summarized_{form}"]를 update함. (클라이언트 별이 아닌 취합된 val_loss 기반으로 업데이트 될 예정) results_type=f"client_summarized_{form}"
    ...
    def check_buffer(self,
        ...
        if check_eval_result:
            ...
            if self._cfg.asyn.use and self._cfg.asyn.aggregator == 'time_up': #self._cfg.asyn.use=False라 무시
    ...
    def terminate(self, msg_type='finish'): #run에서 쓰임.
        """
    ...
    def callback_funcs_model_para(self, message: Message): #클라이언트가 로컬 훈련 후 보낸 model parameter를 수신하고, 이를 서버의 버퍼에 저장하거나 직접 집계하며 집계 조건이 충족되면 다음 라운드로 진행
        ...
        # dequantization
        if self._cfg.quantization.method == 'uniform': #none, 무시해도 됨
            from federatedscope.core.compression import \
        ...
        if self._cfg.federate.online_aggr: #무시해도 됨
        ...
        if self._cfg.asyn.use and self._cfg.asyn.broadcast_manner == \
                'after_receiving': #self._cfg.asyn.uses는 False라 무시
            self.broadcast_model_para(msg_type='model_para',
                                      sample_client_num=1)
