        ...
        if 'ss' in message.msg_type: #msg_type 에 'ss' 포함 ⇒ 비밀 분할 조각(fragment) 수신 처리. 해당 안함!!
            ...
            if self.is_unseen_client or skip_train_isolated_or_global_mode: #해당 안함
                ...
                if self._is_main_process():
                    self._ensure_outdir()
                    agg = getattr(self.trainer.ctx, "eval_metrics", {}) or {}
                    train_agg = {k: v for k, v in agg.items() if k.startswith("train_")}
                    if train_agg:
                        # 모니터 포맷(기존 파이프라인 유지; eval_results.raw 갱신)
                        formatted = self._monitor.format_eval_res(
                            train_agg, rnd=self.state, role=f'Client #{self.ID}',
                            forms=['raw'], return_raw=True)
                        # exp_results.raw에도 같은 한 줄 append (요구사항)
                        self._append_raw_line(f'Client #{self.ID}', self.state,
                                              formatted['Results_raw'], fname="exp_results.raw")
    ...
    def callback_funcs_for_evaluate(self, message: Message): #서버가 “평가 요청(evaluate)” 메시지를 보낼 때, 로컬 데이터에 대해 평가를 수행하고 결과(metrics)를 서버에 보냅니다.
        ...
        if is_main:
            ...
            formatted_eval_res = self._monitor.format_eval_res(
                metrics, rnd=self.state, role=f'Client #{self.ID}',
                forms=['raw'], return_raw=True)
            # exp_results.raw에도 동일 라인 append
            self._append_raw_line(f'Client #{self.ID}', self.state,
                                  formatted_eval_res['Results_raw'], fname="exp_results.raw")
