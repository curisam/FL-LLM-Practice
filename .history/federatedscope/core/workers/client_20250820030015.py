...
class Client(BaseClient):
    ...
    def _log_split_metrics(self, role_str, round_idx, split, trainer_ctx):
        ...
        self.logger.info({
            ...
            'Results': local
        })#INFO: {'Role': 'Client #44', 'Round': 0, 'Split': 'train', 'Rank': '0/4', 'Local': True, 'Results': {'train_total': 120, 'train_loss': 85.08743250370026, 'train_avg_loss': 0.7090619375308355, 'train_acc': 0.0}}
        ...
        if world > 1 and rank == 0:
            ...
            if per_procs:
                ...
                }) #INFO: {'Role': 'Client #44', 'Round': 0, 'Split': 'train', 'Local': False, 'Results_procs': ['train_total', 'train_loss', 'train_avg_loss', 'train_acc', 'rank', 'train_total', 'train_loss', 'train_avg_loss', 'train_acc', 'rank', 'train_total', 'train_loss', 'train_avg_loss', 'train_acc', 'rank', 'train_total', 'train_loss', 'train_avg_loss', 'train_acc', 'rank']}
        ...
        if agg_clean and rank == 0:
            ...
            })#INFO: {'Role': 'Client #44', 'Round': 0, 'Split': 'train', 'Aggregated': True, 'Results_raw': {'train_total': 480, 'train_loss': 348.768310546875, 'train_avg_loss': 0.7266006469726562, 'train_acc': 0.49375}}
    ...
    def callback_funcs_for_model_para(self, message: Message):
        ...
        sample_size, model_para_all, results = self.trainer.train(round_num=self.state)  # 이 실제 한 에폭(batch) 치 로컬 업데이트를 실행하고, sample_size (총 샘플 수), model_para_all (업데이트된 state_dict), results (로컬 로그: loss, acc 등) 리턴
    ...
    def callback_funcs_for_evaluate(self, message: Message):
        ...
        logger.info(f"[DEBUG] is_main={is_main}, raw ctx.eval_metrics={raw_ctx}, merged metrics init={metrics}")
