    ...
    def _register_default_handlers(self):
        ...
        Note:
          ...
          function are shown below:
            ...
            ============================ ==================================
            ``assign_client_id``         ``callback_funcs_for_assign_id()``  #씀.
            ``ask_for_join_in_info``     ``callback_funcs_for_join_in_info()`` #쓸 일 없음
            ``address``                  ``callback_funcs_for_address()``  #쓸 일 없음
            ``model_para``               ``callback_funcs_for_model_para()`` ################# 중요 #################
            ``ss_model_para``            ``callback_funcs_for_model_para()`` #쓸 일 없음
            ``evaluate``                 ``callback_funcs_for_evaluate()``   ################# 중요 #################
            ``finish``                   ``callback_funcs_for_finish()``     ################# 중요 #################
            ``adapter_eval``             ``callback_funcs_for_adapter_eval()``     ################# 중요 #################
            ``adapter_evalset_active_adapter_idx`` ``callback_funcs_for_setting_adapter_idx()``     ################# 중요 #################

            ``converged``                ``callback_funcs_for_converged()`` #쓸 일 없음
            ============================ ==================================
        """
