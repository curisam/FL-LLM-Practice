...
class Server(BaseServer): #클라이언트 등록, 메시지 수신 및 응답 처리, 모델 평가용 사본 생성 등 실제 FL 서버 기능 구현
  ...
  File "/home/seongyoon/jupyter/FedBiscuit/federatedscope/core/workers/server.py", line 759, in merge_eval_results_from_all_clients
    formatted_logs_all_set.update(formatted_logs)
NameError: name 'formatted_logs_all_set' is not defined
