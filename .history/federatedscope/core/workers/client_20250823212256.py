...
class Client(BaseClient):
    ...
    def callback_funcs_for_model_para(self, message: Message):
        ...
        if train_agg: # ✅ rank0(=main process)에서만 집계본을 파일로 기록
    ...
    def _rank_world_now(self):
        import os
        # 기본값
        rank, world = None, None

        # 1) torch.distributed가 살아있으면 그 값을 최우선 사용
        try:
            import torch.distributed as dist
            if dist.is_available() and dist.is_initialized():
                rank  = dist.get_rank()
        ...
        # 2) accelerate가 있으면 보조로 사용
        if (rank is None or world is None):
            acc = getattr(self.trainer, "accelerator", None)
            if acc is not None:
                if rank  is None: rank  = int(getattr(acc, "process_index", 0))
                if world is None: world = int(getattr(acc, "num_processes", 1))

        # 3) 환경변수 최후의 보루
        if rank  is None:
            rank = int(os.environ.get("RANK") or
                    os.environ.get("PMI_RANK") or
                    os.environ.get("SLURM_PROCID") or 0)
        if world is None:
            world = int(os.environ.get("WORLD_SIZE") or
                        os.environ.get("PMI_SIZE") or
                        os.environ.get("SLURM_NTASKS") or 1)
    ...
    def callback_funcs_for_evaluate(self, message: Message):
        ...
        rank, world = self._rank_world_now()
        import os, sys
        print(f"[EVAL START] pid={os.getpid()} rank={rank}/{world} "
            f"acc={'Y' if getattr(self.trainer,'accelerator',None) else 'N'}",
