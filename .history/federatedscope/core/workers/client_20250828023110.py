...
class Client(BaseClient):
    ...
    def callback_funcs_for_model_para(self, message: Message):
        ...
        sample_size, model_para_all, results = self.trainer.train(round_num=self.state)  # 전체 train data 갯수, model_para_all (adapter 1개라 이것은 active adapter의 state_dict), split routine 돌며 집계된 results (num_total, loss, acc 등) 리턴
