...
class Client(BaseClient):
    ...
    def callback_funcs_for_model_para(self, message: Message):
        ...
        if self.early_stopper.early_stopped and \
                self._monitor.local_convergence_round == 0: #일반적으로 False라 Pass!!
            logger.info(
        ...
        sample_size, model_para_all, results = self.trainer.train(round_num=self.state)  # #split 거친 전체 데이터 수, model_para_all (업데이트된 state_dict), split routine 돌며 집계된 results (num_total, loss, acc 등) 리턴
