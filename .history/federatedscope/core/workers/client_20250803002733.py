...
class Client(BaseClient):
    ...
    #     # 멀티프로세스일 때 rank0에서만 파일 기록/서버 전송
    #     is_main = self._is_main_process()

    #     if is_main:
    #         self._ensure_outdir()

    #         # trainer.ctx.eval_metrics 에서 test_/val_ 접두사만 추출 (집계본)
    #         ctx = getattr(self.trainer, "ctx", None)
    #         agg_all = getattr(ctx, "eval_metrics", {}) if ctx is not None else {}
    #         eval_agg = {k: v for k, v in (agg_all or {}).items()
    #                     if k.startswith("test_") or k.startswith("val_")}

    #         # === 파일 기록: eval_results.raw ===
    #         if eval_agg:
    #             self._append_raw_line(
    #                 role_str=f"Client #{self.ID}",
    #                 round_idx=self.state,
    ...
    #         # 베스트/히스토리 갱신도 eval_agg 기준으로
    #         if eval_agg:
    #             updated = self._monitor.update_best_result(
    #                 self.best_results, eval_agg, results_type=f"client #{self.ID}"
    #             )
    #             if updated and self._cfg.federate.save_client_model:
    #                 path = add_prefix_to_path(f'client_{self.ID}_', self._cfg.federate.save_to)
    #                 if self._is_main_process():
    ...
    #             # 라운드별 히스토리 누적 및 early stop 체크(키 존재할 때만)
    #             self.history_results = merge_dict_of_results(self.history_results, eval_agg)
    #             key_for_es = self._cfg.eval.best_res_update_round_wise_key
    #             if key_for_es in self.history_results:
    #                 self.early_stopper.track_and_check(self.history_results[key_for_es])

    #     # 서버로는 rank0만 전송(중복 방지)
    #     if is_main:
    #         self.comm_manager.send(
    #             Message(msg_type='metrics',
    #                     sender=self.ID,
    ...
    #     # === test+val 합본을 한 줄로 파일/로그에 (rank0만) ===
    #     is_main = self._is_main_process()

    #     if is_main:
    #         self._ensure_outdir()
    #         # 모니터 포맷 (eval_results.raw 갱신)
    #         formatted_eval_res = self._monitor.format_eval_res(
    #             metrics, rnd=self.state, role=f'Client #{self.ID}', forms=[], return_raw=True)
            
    #         # ✅ 평가 루프가 끝난 직후(두 split 모두 끝난 시점)
    #         if self._is_main_process():
    #             ctx = getattr(self.trainer, "ctx", None)
    #             agg_all = getattr(ctx, "eval_metrics", {}) if ctx is not None else {}
    #             # test_, val_ 접두사만 모아 합본으로 저장
    #             eval_agg = {k: v for k, v in (agg_all or {}).items()
    #                         if k.startswith("test_") or k.startswith("val_")}
    #             if eval_agg:
    #                 self._append_raw_line(
