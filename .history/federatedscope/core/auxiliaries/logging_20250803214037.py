...
def setup_logging(cfg) -> None:
    ...
    - Do NOT create sub_exp_*; use cfg.outdir as-is.
    - Add FileHandler only on rank0.
    - Always keep a StreamHandler for console.
    """
    # 1) outdir 고정 (sub_exp 금지)
    root_outdir = _ensure_outdir(cfg.outdir)
    cfg.outdir = root_outdir
    ...
    if _is_main_process():
        ...
        if not has_file:
            log_path = os.path.join(cfg.outdir, "exp_print.log")
            fh = logging.FileHandler(log_path, mode="a", encoding="utf-8")
            fh.setLevel(logging.INFO)
            fh.setFormatter(fmt)
            root.addHandler(fh)
    else:
        # 비-메인은 혹시 달려있는 FileHandler 제거
...
def update_logging_level(level: int) -> None:
    logging.getLogger().setLevel(level)
    for h in logging.getLogger().handlers:
        h.setLevel(level)

# ----------------------------------------------------------
# wandb helpers (기존 인터페이스 유지)
# ----------------------------------------------------------
def logline_2_wandb_dict(exp_stop_normal: bool, line: str,
                         log_res_best: Dict[str, Any],
                         raw_out: bool = False) -> Tuple[bool, Dict[str, Any]]:
    """
    기존 코드가 의존하는 최소 기능만 유지:
    - 한 줄 로그를 eval 가능하면 dict로, 아니면 {"text": line} 로 래핑
    - best 결과는 갱신된 dict를 그대로 넘겨줌
    """
    try:
        obj = eval(line.strip())  # nosec - 내부 로그 파서 용도
        if isinstance(obj, dict):
            log_res = obj
        else:
            log_res = {"text": str(obj)}
    except Exception:
        log_res = {"text": line.strip()}
    # best 요약(요청하는 곳에서 summary에 덮어씀)
    for k, v in log_res.items():
        if isinstance(v, (int, float)):
            log_res_best[k] = v
    return exp_stop_normal, log_res

def logfile_2_wandb_dict(f, raw_out: bool = False) -> Tuple[List[Dict[str, Any]], bool, str, Dict[str, Any]]:
    """
    eval_results.log 같은 파일을 읽어 wandb로 넘길 수 있는 dict 리스트로 변환.
    """
    ...
    for line in f:
        ...
        try:
            ...
            if isinstance(obj, dict):
                ...
                logs.append({"text": str(obj)})
        except Exception:
            logs.append({"text": line.strip()})
    return logs, exp_stop_normal, last_line, best
