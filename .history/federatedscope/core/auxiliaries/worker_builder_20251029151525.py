...
def get_client_cls(cfg):
    ...
    from federatedscope.llm.llm_local.moe_server_client import FullMoEClient, FusionMoEClient
    
    # 1) LLM MoE 클라이언트타입이 명시돼 있으면 그에 맞는 클라이언트 클래스 반환
    client_type = getattr(cfg.federate, 'client_type', '').lower()
    if client_type == 'llmfullmoeclient':
        return FullMoEClient
    elif client_type == 'llmfusionmoeclient':
        return FusionMoEClient
    ...
    if cfg.federate.method.lower() in constants.CLIENTS_TYPE: ######################cfg.federate.method.lower()=FedAvg로 여기 걸린다.############################
        client_type = constants.CLIENTS_TYPE[cfg.federate.method.lower()]  ## client_type=normal ##
    else:
        client_type = "normal"
        logger.warning(
    ...
    if cfg.llm.adapter.local_only or cfg.llm.adapter.count > 1: ######################이것만 걸린다.############################
...
def get_server_cls(cfg):
    ...
    from federatedscope.llm.llm_local.moe_server_client import FullMoEServer, FusionMoEServer
    
    # 1) LLM MoE 서버타입이 명시돼 있으면 그에 맞는 서버 클래스를 반환
    server_type = getattr(cfg.federate, 'server_type', '').lower()
    if server_type == 'llmfullmoeserver':
        return FullMoEServer
    elif server_type == 'llmfusionmoeserver':
        return FusionMoEServer



    for func in register.worker_dict.values(): #아무것도 안걸린다
        worker_class = func(cfg.federate.method.lower())
        if worker_class is not None:
    ...
    if cfg.federate.method.lower() in constants.SERVER_TYPE: ######################이것만 걸린다.############################
        server_type = constants.SERVER_TYPE[cfg.federate.method.lower()] ## normal ##
    else:
        server_type = "normal"
        logger.warning(
