...
def get_trainer(model=None,
    ...
    if server_type == 'llmfullmoeserver':
        from federatedscope.llm.trainer.full_moe_trainer import FullMoETrainer
        return FullMoETrainer(model, data, device, config, only_for_eval=False, monitor=monitor)
    if server_type == 'llmfusionmoeserver':
        from federatedscope.llm.trainer.fusion_moe_trainer import FusionMoETrainer
        return FusionMoETrainer(model, data, device, config, only_for_eval=False, monitor=monitor)





    if config.trainer.type == 'general': #해당 안함
        ...
        return None
    elif config.trainer.type.lower() in TRAINER_CLASS_DICT: #TRUE
        ...
        ###########################################################################
        elif config.trainer.type.lower() in ['llmrewardchoicetrainer']: #TRUE
        ...
        trainer_cls = getattr(importlib.import_module(name=dict_path),
                              TRAINER_CLASS_DICT[config.trainer.type.lower()]) #RewardChoiceTrainer
    ...
    elif 'backdoor' in config.attack.attack_method: #False
        logger.info(
