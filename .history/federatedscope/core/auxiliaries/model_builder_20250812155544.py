...
def get_model(config, local_data=None, backend='torch'):
    ...
    if model_config.type.lower() in \
            ...
            model_config.type.lower().endswith('_llm'): #True
        input_shape = None
    ...
    for func in register.model_dict.values(): #아무것도 안걸린다.
    ...
    elif model_config.type.lower().endswith('_llm'): ################ 이것만 걸린다 ##################
        from federatedscope.llm.model import get_llm
        model = get_llm(config)#peft 적용, adapter 3개
