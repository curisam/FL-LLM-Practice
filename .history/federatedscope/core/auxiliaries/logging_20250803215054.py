...
- 콘솔 로그는 모든 rank에서 출력(원하면 rank>0 숨기는 옵션 추가 가능)
...
def update_logger(
    ...
    - cfg.outdir 하위에 exp_print.log를 rank0만 기록(only_main_file=True 기준)
    - 콘솔 핸들러는 모든 rank에 장착
    - 기존 핸들러 제거 후 재설정(중복 방지)
    - main.py의 추가 인자 호환: clear_before_add 등은 수용하되 동작상 문제 없도록 처리
    """
    # 루트 로거
    root = logging.getLogger()

    # 핸들러 초기화: clear_before_add가 True든 아니든, 항상 모두 제거 후 재설정
