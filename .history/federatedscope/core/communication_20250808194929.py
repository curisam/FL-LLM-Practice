...
class StandaloneCommManager(object):
    ...
    def __init__(self, comm_queue, monitor=None):
        ...
        """
        comm_queue
            서버와 클라이언트가 공유하는 메시지 큐(collections.deque 또는 queue.Queue).

            모든 send() 호출이 이 큐에 메시지를 밀어넣고, 상대방이 꺼내 가공합니다.
        neighbors

            이 통신 관리자가 “연결된” 이웃 워커들의 ID→주소(또는 식별자) 매핑을 담는 딕셔너리.

        monitor

            통신량(바이트) 추적 등을 위해 외부에서 주입할 수 있는 콜백 객체.

        """


        """
        클라이언트가 학습을 끝내고 서버에게 model_para를 보내는 경우:


        msg = Message(sender=client_id, receiver=0, msg_type="model_para", content=...)
        comm_manager.send(msg)       
                
        → 내부에서:
        comm_queue.append(msg)



        🧠 목적:
        메시지 기반 아키텍처를 구현함으로써 FL 시스템을 비동기적이지만 순차적으로 처리 가능하게 함.

        StandaloneRunner나 Server는 이 큐에서 메시지를 pop하면서 순서대로 처리.

        ✅ 예시 흐름:

        Client.train -> send(model_para) -> comm_queue.append(...)
        Server._run_simulation -> msg = comm_queue.popleft() -> handle_message(msg)
