...
class GeneralTorchTrainer(Trainer):
    ...
    def update(self, model_parameters, strict=False):
        ...
        if is_ddp and len(server_params) > 0:
            ...
            local_keys = self.ctx.model.state_dict().keys() #module. 뿥어있음. self.ctx.model은 클라이언트가 가장 최근에 만든 local 모델.
            ...
            if matching_local_key:
                ...
                #prefix_to_add= module.
                prefix_to_add = matching_local_key[:prefix_len]  #서버 키: base_model.layers.0.q_proj.lora_A.default.weight, 로컬 키: module.base_model.layers.0.q_proj.lora_A.default.weight → endswith 성립
            
            if prefix_to_add: #module.를 서버키에 부착.
