...
class Trainer(BaseTrainer):
    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        ...
        if mode == MODE.TRAIN:
            # ctx 변수 이름에 split이 붙는 형태와 안 붙는 형태 모두 고려
            num_samples_s = self.ctx.get(f'num_samples_{split}', 0)
            num_samples_ns = self.ctx.get('num_samples', 0)
            num_samples = num_samples_s or num_samples_ns

            if num_samples > 0:
                loss_total_s = self.ctx.get(f'loss_total_{split}', 0.0)
                loss_total_ns = self.ctx.get('loss_total', 0.0)
                loss_total = loss_total_s or loss_total_ns

                correct_s = self.ctx.get(f'correct_{split}', 0)
                correct_ns = self.ctx.get('correct', 0)
                correct = correct_s or correct_ns
                ...
                local_results = {
                    ...
                    f'{split}_correct': correct # gather를 위해 correct도 추가
                }
