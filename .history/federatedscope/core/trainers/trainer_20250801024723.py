...
class Trainer(BaseTrainer):
    ...
    def train(self, target_data_split_name="train", hooks_set=None):
        """
        Run local training given the client's data and the received model.
        This method is modified to support distributed training via `accelerate`.
        It aggregates results from all processes before returning.

        Returns:
            tuple: A tuple containing:
              - (int): The number of samples processed.
              - (dict): The model parameters.
              - (dict): A dictionary containing training results (e.g., loss, accuracy),
                        globally aggregated across all processes.
        """
        hooks_set = hooks_set or self.hooks_in_train
        self.ctx.check_split(target_data_split_name)

        # 1. _run_routine을 호출하여 각 프로세스에서 로컬 학습 수행
        #    이 함수가 끝나면 self.ctx에 로컬 결과가 누적됩니다.
        num_samples = self._run_routine(MODE.TRAIN, hooks_set,
                                        target_data_split_name)

        # 2. Monitor를 사용해 로컬 누적 결과를 깔끔한 딕셔너리로 변환
        #    이 작업은 메모리 정리 훅이 실행되기 전에 수행되어야 합니다.
        local_results = self.ctx.monitor.eval(self.ctx)

        # 3. (분산 환경일 경우) 모든 프로세스의 로컬 결과를 통합
        if hasattr(self, 'accelerator') and self.accelerator is not None \
                and self.accelerator.num_processes > 1:
            
            # 모든 프로세스로부터 local_results 딕셔너리를 수집
            gathered_results = self.accelerator.gather_for_metrics(local_results)

            # 메인 프로세스에서만 최종 글로벌 결과 계산
            if self.accelerator.is_main_process:
                # Client 로그 형식과 일치하도록 키 이름을 'train_...'으로 사용
                split = self.ctx.cur_split  # 'train'
                
                # 모든 프로세스의 결과를 합산
                total_loss = sum(m.get(f'{split}_loss', 0) for m in gathered_results)
                total_samples = sum(m.get(f'{split}_total', 0) for m in gathered_results)

                if total_samples > 0:
                    avg_loss = total_loss / total_samples
                    # 가중 평균 정확도 계산: (acc * total)의 합 / 전체 total
                    weighted_acc_sum = sum(m.get(f'{split}_acc', 0) * m.get(f'{split}_total', 0) for m in gathered_results)
                    ...
                    global_results = {
                        ...
                        f'{split}_acc': avg_acc,
                    }
                else:
                    global_results = {}
                
                # 최종 결과를 self.ctx.eval_metrics에 저장
                self.ctx.eval_metrics = global_results
            else:
                # 다른 프로세스는 결과 비우기
        ...
        # 4. 최종적으로 모델 파라미터와 (통합된) 결과 딕셔너리를 반환
        return num_samples, self.get_model_para(), self.ctx.eval_metrics
    # =========================================================================
