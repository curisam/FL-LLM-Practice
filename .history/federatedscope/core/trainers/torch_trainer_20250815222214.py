...
class GeneralTorchTrainer(Trainer):
    ...
    def get_model_para(self): #active adapter para만 떼온다.
                ...
                if data.get(split, None) is not None:
                    if isinstance(data.get(split), Dataset):
                        ...
                        init_dict["num_{}_data".format(split)] = len(
                            data.get(split))
                    elif isinstance(data.get(split), DataLoader):###### ##이거에만 해당할 듯.
    ...
    def update(self, model_parameters, strict=False): #서버 모델 브로드캐스팅.
        """
            Called by the FL client to update the model parameters
        Arguments:
            model_parameters (dict): PyTorch Module object's state_dict.
