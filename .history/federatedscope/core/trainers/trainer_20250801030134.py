...
class Trainer(BaseTrainer):
    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        ...
        # --- [핵심 수정] Train 모드일 때, MetricCalculator를 우회하여 결과 딕셔너리 생성 ---
        if mode == MODE.TRAIN:
            ...
            if num_samples > 0:
                ...
                # train()이 반환할 결과 딕셔너리를 ctx.eval_metrics에 수동으로 채움
        ...
        # 분산 환경일 경우, 위에서 만든/기존에 있던 eval_metrics를 통합
        if hasattr(self, 'accelerator') and self.accelerator.num_processes > 1:
            # 이제 self.ctx.eval_metrics는 train 모드에서도 올바른 값을 가짐
        ...
        # 단일 프로세스 환경에서 eval/test 모드일 경우, monitor.eval을 호출해야 함
        # (train 모드는 위에서 이미 처리됨)
