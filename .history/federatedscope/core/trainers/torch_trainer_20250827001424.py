...
class GeneralTorchTrainer(Trainer):
    ...
    def get_model_para(self): #active adapter para만 떼온다.
        if self.cfg.federate.process_num > 1 or \
            ...
            # ._param_filter  통해 바뀌는 거 없음. self.ctx.model.state_dict() 반환. #requires_grad=True인 파라미터 포함인 것 혹은 self.adapter_names에 있는 어댑터 이름이 파라미터 이름 문자열에 포함되면, requires_grad=False라도 포함. 
