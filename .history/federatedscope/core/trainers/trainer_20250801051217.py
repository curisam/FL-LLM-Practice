...
class Trainer(BaseTrainer):
    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        # ... (함수 앞부분은 기존과 동일: track_mode, track_split, on_fit_start, _run_epoch)
        ...
        # --- [핵심 수정] accelerator 객체 확인 및 생성 ---
        # Trainer에 accelerator가 없으면, 여기서 직접 생성 시도
        if not hasattr(self, 'accelerator'):
            # llm.accelerator.use 설정이 True일 때만 생성
            if self.cfg.llm.get('accelerator', {}).get('use', False) and Accelerator is not None:
                self.accelerator = Accelerator()
            else:
                self.accelerator = None

        # 2. 분산 환경일 경우, 로컬 결과를 통합
        using_accelerate = (self.accelerator is not None and self.accelerator.num_processes > 1)
