...
class Trainer(BaseTrainer):
    ...
    def train(self, target_data_split_name="train", hooks_set=None):
        ...
        # 2. **[핵심 수정]** train 모드에서는 monitor.eval() 대신 ctx에서 직접 결과 생성
        split = self.ctx.cur_split  # 'train'
        
        # ctx에서 로컬 누적 결과를 직접 읽어옴
        local_num_samples = self.ctx.get(f'num_samples_{split}', 0)
        local_results = {}
        if local_num_samples > 0:
            loss_total = self.ctx.get(f'loss_total_{split}', 0.0)
            correct = self.ctx.get(f'correct_{split}', 0)
            local_results = {
                f'{split}_total': local_num_samples,
                f'{split}_loss': loss_total,
                f'{split}_avg_loss': loss_total / local_num_samples,
                f'{split}_acc': correct / local_num_samples,
            }

        # 3. 분산 환경(accelerate) 여부 확인 및 결과 통합
