...
class Trainer(BaseTrainer):
    ...
    def _param_filter(self, state_dict, filter_keywords=None): #서버로 보낼 파라미터를 필터링하기 위함.즉, 개인화 FL(Personalized FL)에서 로컬에만 남길 파라미터를 빼고, 전송 가능한 파라미터만 골라내는 역할.
        ...
        return dict(
            filter(
                lambda elem: trainable_filter(elem[1]) and keyword_filter(
                    elem[0], filter_keywords), state_dict.items()) #filter_keywords=[]-> keyword_filter(elem[0], filter_keywords) 언제나 True.
                    
                    
                    state_dict.items()에 대한 input elem에 대해서  "trainable_filter(elem[1]) and keyword_filter(elem[0], filter_keywords)" Boolean을 대응하는 함수.
