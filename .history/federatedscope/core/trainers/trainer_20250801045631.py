...
class Trainer(BaseTrainer):
    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        """
        Runs a routine (train, val, test).
        This method is modified to correctly handle result aggregation in
        distributed training environments (e.g., using `accelerate`),
        while being robust against memory cleaning hooks (`on_fit_end`).
        """
        ...
        # === [핵심] on_fit_end 훅이 실행되기 "전에" 로컬 결과를 먼저 계산하여 백업 ===
        
        split = self.ctx.cur_split
        local_results = {}
        
        # 1. 모드에 따라 로컬 결과 딕셔너리를 생성합니다.
        if mode == MODE.TRAIN:
            # train 모드에서는 ctx에 직접 누적된 값들을 읽어와 딕셔너리를 구성합니다.
            # .get()을 사용하여 해당 속성이 없어도 에러 없이 안전하게 기본값을 가져옵니다.
            ...
            # 이 시점에는 ctx.ys_true, ctx.ys_pred 등이 아직 삭제되지 않았습니다.
            local_results = self.ctx.monitor.eval(self.ctx)

        # on_fit_end 훅 (메모리 정리 훅 포함)을 이제 실행합니다.
        # 이 훅은 ctx의 중간 계산 값들을 삭제할 수 있지만, 우리는 이미 local_results에 백업해두었습니다.
        for hook in hooks_set["on_fit_end"]:
            hook(self.ctx)

        # 2. 분산 환경일 경우, 백업해 둔 로컬 결과를 통합하고 모든 프로세스에 전파합니다.
        using_accelerate = (hasattr(self, 'accelerator') and 
                            self.accelerator is not None and
                            self.accelerator.num_processes > 1)

        if using_accelerate:
            # 모든 프로세스로부터 local_results 딕셔너리 객체를 수집합니다.
            gathered_results_list = gather_object(local_results)
            
            # 모든 프로세스가 동일한 최종 글로벌 결과를 계산합니다.
            final_results = {}
            if gathered_results_list: # 리스트가 비어있지 않은지 확인
                # 모든 프로세스의 결과를 합산합니다.
                ...
                if total_samples > 0:
                    ...
                    # 정확도는 가중 평균으로 계산합니다: (acc * total)의 합 / 전체 total
            ...
            # 계산된 최종 결과를 모든 프로세스의 ctx.eval_metrics에 저장합니다.
            self.ctx.eval_metrics = final_results
                
        else:
            # 단일 프로세스 환경에서는 백업해 둔 로컬 결과를 그대로 최종 결과로 사용합니다.
