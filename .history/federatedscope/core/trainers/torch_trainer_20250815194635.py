...
class GeneralTorchTrainer(Trainer):
    ...
    #             self.cfg.llm.accelerator.use: #True, True, False, True
            

    #         return self._param_filter(self.ctx.model.state_dict()) #학습가능한 파라미터만 필터링해서 보냄.
    #     else:
    #         return self._param_filter(self.ctx.model.cpu().state_dict()) #학습가능한 파라미터만 필터링해서 보냄.
    ...
    def get_model_para(self):
        if self.cfg.federate.process_num > 1 or \
            ...
            # 바뀌는 거 없음. self.ctx.model.state_dict() 반환. active adpater의 것만. 필터링을 먼저하고 키를 수정하는 순서가 올바릅니다.
    ...
    #     # 1) 받은 파라미터들을 모두 텐서로 변환. 이렇게 해야 load_state_dict 등에서 타입 오류가 나지 않기 위함.
    #     for key in model_parameters:
    #         model_parameters[key] = param2tensor(model_parameters[key]) #param2tensor 는, NumPy array 나 다른 형태로 넘어온 파라미터를 torch.Tensor 로 보장해 주는 헬퍼입니다.
    ...
    #     #self.ctx.model.state_dict().copy() 는 현재 로컬 모델 파라미터(딕셔너리)를 복사해 옵니다.
        
    #     # self._param_filter(model_parameters) 는, 받은 전체 파라미터 중에서 이 클라이언트 모델에 실제로 적용해야 할 부분만 필터링 해 주는 함수. 
    #     # (예: llm.adapter 만 골라오거나, shared vs local parameters 구분 등)

    #     # merge_param_dict 는 두 개의 state_dict 를 키 단위로 병합하되, 새로 받은 쪽이 우선권을 갖도록 덮어 씌웁니다.
    ...
    #     #strict=True->모델이 기대하는 모든 키들이 merged_param에 있어야 함. merged_param에 모델에 없는(예상치 못한) 키가 하나라도 없어야 합니다.
    #     #strict=False->모델에 없는 키(unexpected_keys)는 그냥 무시하고 버립니다. merged_param에 없는 모델 내 키(missing_keys)는 그대로 초기값(또는 이전에 있던 값) 을 유지합니다.
    ...
    def update(self, model_parameters, strict=False):
        ...
        server_params = {k: param2tensor(v) for k, v in model_parameters.items()}

        # 2) DDP 환경인지 확인하고, 그렇다면 서버 파라미터의 키를 수정
        is_ddp = isinstance(self.ctx.model, torch.nn.parallel.DistributedDataParallel)
        if is_ddp and len(server_params) > 0:
            logger.info("DDP environment detected. Adding 'module.' prefix to incoming server keys.")
            
            # 동적 접두사 탐지 (더 안정적)
            # 로컬 모델 키와 서버 키를 비교하여 필요한 접두사를 동적으로 찾습니다.
            local_keys = self.ctx.model.state_dict().keys()
            sample_server_key = next(iter(server_params.keys()))
            matching_local_key = next((k for k in local_keys if k.endswith(sample_server_key)), None)

            prefix_to_add = ''
            if matching_local_key:
                prefix_len = len(matching_local_key) - len(sample_server_key)
                prefix_to_add = matching_local_key[:prefix_len]
            
            if prefix_to_add:
                server_params = {f'{prefix_to_add}{k}': v for k, v in server_params.items()}
                if self.ctx.rank == 0:
                    logger.info(f"Keys modified with prefix '{prefix_to_add}'.")
            
        # 3) (핵심) 키가 수정된 서버 파라미터를 모델에 직접 로드
        # strict=False이므로, server_params에 있는 LoRA 파라미터만 업데이트되고
        # 나머지 베이스 모델 파라미터는 그대로 유지됩니다.
        # merge_param_dict나 _param_filter를 여기서 호출할 필요가 없습니다.
        self.ctx.model.load_state_dict(server_params, strict=strict)    
    ...
    # def _hook_on_batch_start_init(self, ctx): #배치별로 사용될 통계 변수(loss_batch, y_true, y_prob, batch_size 등)를 CtxVar(..., LIFECYCLE.BATCH)로 초기화. 이후 바로 on_batch_forward 훅이 이를 채워넣을 수 있도록 준비.
    #     """
    #     Note:
    #       The modified attributes and according operations are shown below:
    #         ==================================  ===========================
