...
class Trainer(BaseTrainer):
    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        ...
        # on_fit_end 훅 실행 전 결과 처리
        split = self.ctx.cur_split
        local_results = {}
        
        # 1. 로컬 결과 생성
        if mode == MODE.TRAIN:
            # LLMTrainer의 _hook_on_batch_end가 사용하는 변수 이름에 맞춤
            num_samples = self.ctx.get('num_samples', 0)
            if num_samples > 0:
                # LLMTrainer는 loss_batch_total을 사용함
                loss_total = self.ctx.get('loss_batch_total', 0.0) 
                # RewardChoiceTrainer가 추가한 correct를 사용
