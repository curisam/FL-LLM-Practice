...
class Trainer(BaseTrainer):
    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        ...
        # === [최종 수정된 부분 시작] on_fit_end 훅 실행 전 ===
        
        # === [최종 디버깅 버전] on_fit_end 훅 실행 전 ===
        ...
        # 1. 로컬 결과 딕셔너리 생성
        if mode == MODE.TRAIN:
            # [디버깅] ctx의 실제 변수 이름 확인 (num_samples, loss_total, correct)
            # _run_batch에서 어떤 이름으로 ctx에 저장하는지 확인이 필요.
            # 가장 일반적인 이름으로 시도
            ...
            if hasattr(self, 'accelerator'):
                ...
                logger.info(f"[DEBUG PID:{pid}] Raw ctx values: num_samples={num_samples}, loss_total={loss_total}, correct={correct}")
        ...
        if using_accelerate:
            # [수정] gather_for_metrics 대신 gather_object 사용
            gathered_results_list = gather_object(local_results)
            
            # [디버깅 로그 추가]
            if self.accelerator.is_main_process:
                logger.info(f"[DEBUG Main] Gathered results list: {gathered_results_list}")

            # 모든 프로세스가 동일한 최종 결과를 계산
            final_results = {}
            # gather_object는 리스트를 반환하므로, 리스트가 비어있지 않은지 확인
