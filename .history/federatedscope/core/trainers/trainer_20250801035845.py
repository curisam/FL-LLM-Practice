...
class Trainer(BaseTrainer):
    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        ...
        # 2. 분산 환경일 경우, 로컬 결과를 통합하고 "모든 프로세스에 전파"
        using_accelerate = (hasattr(self, 'accelerator') and 
                            self.accelerator is not None and
                            self.accelerator.num_processes > 1)

        if using_accelerate:
            # [수정 1] gather_object 대신 gather_for_metrics 사용 (텐서 기반이 더 안정적)
            # 텐서로 변환 가능한 숫자 값들만 모음
            metrics_to_sync = {k: v for k, v in local_results.items() if isinstance(v, (int, float))}
            gathered_metrics_list = self.accelerator.gather_for_metrics(metrics_to_sync)
            
            # 이제 gathered_metrics_list는 모든 프로세스에 동일하게 존재
            # 예: [{'train_total': 120}, {'train_total': 120}, ...]
            
            # [수정 2] 모든 프로세스가 동일한 최종 결과를 계산
            final_results = {}
            total_samples = sum(res.get(f'{split}_total', 0) for res in gathered_metrics_list)
            
            if total_samples > 0:
                total_loss = sum(res.get(f'{split}_loss', 0) for res in gathered_metrics_list)
                weighted_acc_sum = sum(res.get(f'{split}_acc', 0) * res.get(f'{split}_total', 0) for res in gathered_metrics_list)
            ...
            # [수정 3] 계산된 최종 결과를 모든 프로세스의 ctx.eval_metrics에 저장
