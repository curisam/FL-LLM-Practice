...
if __name__ == "__main__":
    ...
    # (a) 배치 단계에서만 유지할 변수
    ctx.temp_loss = CtxVar(0.42, lifecycle="batch")
...
class LifecycleDict(dict):
    ...
    def clear(self, lifecycle):
        ...
        for key in keys:
            ...
            self.lifecycles[lifecycle].remove(key) #ctx.lifecycles[lifecycle]에서 key도 삭제
...
class Context(LifecycleDict): #LifecycleDict를 상속받아 모델, 설정(cfg), 데이터, 장치(device), 훈련·평가 모드·스플릿 등 FederatedScope 의 학습·평가 파이프라인 전반에 필요한 상태를 한곳에 묶어 두는 저장소
    ...
    def num_train_batch(self):
        ...
        return self._calculate_batch_epoch_num(mode='train')[0] #일반적인 epoch당 batch 개수
    ...
    def _calculate_batch_epoch_num(self, mode='train'):#“local update step” 수, “gradient accumulation” 수, “batch_or_epoch” 설정을 반영. 리턴값은 (한 에폭 배치 수, 마지막 에폭 배치 수, 에폭 수, 전체 배치 수)
        #Val/Test: 에폭 수 고정 1, 배치 수만 데이터 크기 기준으로 계산
        # 1) 사용할 split 결정
    ...
    # # --- check_split 함수를 이 내용으로 완전히 교체 ---
    # def check_split(self, target_split_name, skip=False):
    #     data_exist = hasattr(self, f"{target_split_name}_data") and \
    #         getattr(self, f"{target_split_name}_data") is not None
    #     loader_exist = hasattr(self, f"{target_split_name}_loader") and \
    #         getattr(self, f"{target_split_name}_loader") is not None

    #     # 데이터는 있는데 로더가 없으면, 여기서 즉시 생성
    #     if data_exist and not loader_exist:
    #         dataset = WrapDataset(getattr(self, f"{target_split_name}_data"))
    #         loader = get_dataloader(dataset, self.cfg, target_split_name)
    #         setattr(self, f"{target_split_name}_loader", ReIterator(loader))
    #         loader_exist = True # 이제 로더가 존재함

    #     if not data_exist and not loader_exist:
    #         if skip:
    #             logger.warning(
    #                 f"No {target_split_name}_data or"
    #                 f" {target_split_name}_loader in the trainer, "
