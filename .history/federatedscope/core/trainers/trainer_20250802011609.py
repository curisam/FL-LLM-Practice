...
class Trainer(BaseTrainer):
    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        ...
        ret_num_samples = num_samples  # ← 반환값도 전역 합계로 대체 예정
        ...
        if using_accelerate and num_samples > 0:
            try:
                ...
                if total_all > 0:
                    ...
                    ret_num_samples = total_all     # ← 중요: 서버로 반환할 샘플 수도 전역 합계
                else:
                    self.ctx.eval_metrics = local_results
            except Exception as e:
        ...
        # 5) on_fit_end (여기서 accelerator 삭제/메모리 정리 가능)
        for hook in hooks_set["on_fit_end"]:
            hook(self.ctx)
