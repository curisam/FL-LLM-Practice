...
class Trainer(BaseTrainer):
    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        """
        Runs a routine (train, val, test) and aggregates results in a distributed environment.
        """
        self.ctx.track_mode(mode)
        self.ctx.track_split(dataset_name or mode)

        for hook in hooks_set["on-fit_start"]:
            hook(self.ctx)

        self._run_epoch(hooks_set)

        # on_fit_end 훅이 실행되기 "전에" 모든 결과 처리를 완료합니다.
        
        split = self.ctx.cur_split
        local_results_dict = {}
        
        # 1. 각 프로세스에서 로컬 결과 딕셔너리를 생성합니다.
        if mode == MODE.TRAIN:
            # train 모드에서는 ctx에서 직접 누적된 값을 읽습니다.
            # LLM trainer는 `ctx` 변수 이름이 다를 수 있으므로, .get()으로 안전하게 접근합니다.
            num_samples = self.ctx.get(f'num_samples_{split}', self.ctx.get('num_samples', 0))
            if num_samples > 0:
                loss_total = self.ctx.get(f'loss_total_{split}', self.ctx.get('loss_total', 0.0))
            ...
            # eval/test 모드에서는 monitor.eval()을 통해 결과를 생성합니다.
            raw_eval_metrics = self.ctx.monitor.eval(self.ctx)
            # 키 이름에서 'test_'/'val_' 접두사를 제거하여 통일합니다.
            local_results_dict = {
                'total': raw_eval_metrics.get(f'{split}_total', 0),
                'loss': raw_eval_metrics.get(f'{split}_loss', 0.0),
                'correct': raw_eval_metrics.get(f'{split}_correct', 0)
            }
        ...
        if using_accelerate:
            # 2a. 로컬 결과를 [total, loss, correct] 순서의 텐서로 변환합니다.
            if local_results_dict:
                data_to_gather = torch.tensor(
                    [local_results_dict.get('total', 0), 
                     local_results_dict.get('loss', 0.0), 
                     local_results_dict.get('correct', 0)],
                    dtype=torch.float32, # 데이터 타입을 통일하여 gather 안정성 높임
                    device=self.accelerator.device
                )
            else:
                data_to_gather = torch.zeros(3, dtype=torch.float32, device=self.accelerator.device)

            # 2b. 모든 프로세스로부터 1차원 텐서들을 이어붙여 받습니다.
            #    결과는 (num_processes * 3) 크기의 1차원 텐서가 됩니다.
            gathered_tensors_flat = self.accelerator.gather(data_to_gather)
            
            # 2c. [핵심] 1차원 텐서를 (num_processes, 3) 모양의 2차원 행렬로 재구성합니다.
            num_processes = self.accelerator.num_processes
            gathered_tensors = gathered_tensors_flat.view(num_processes, 3)
            
            # 2d. 2차원 행렬에서 각 열(column)을 합산하여 최종 글로벌 결과를 계산합니다.
            total_samples = gathered_tensors[:, 0].sum().item()
            total_loss = gathered_tensors[:, 1].sum().item()
            total_correct = gathered_tensors[:, 2].sum().item()
            ...
            if total_samples > 0:
                final_results = {
                    ...
                    f'{split}_correct': total_correct, # correct는 acc 계산에만 사용
                    f'{split}_avg_loss': total_loss / total_samples,
                    f'{split}_acc': total_correct / total_samples
                }
            
            # 최종 통합 결과를 모든 프로세스의 ctx.eval_metrics에 저장합니다.
            self.ctx.eval_metrics = final_results
                
        else: # 단일 프로세스 환경
            # 로컬 결과를 최종 결과로 사용하되, 키 이름에 split 접두사를 붙여줍니다.
            self.ctx.eval_metrics = {}
            if local_results_dict:
                 self.ctx.eval_metrics = {f'{split}_{k}': v for k, v in local_results_dict.items()}
                 if local_results_dict.get('total', 0) > 0:
                     self.ctx.eval_metrics[f'{split}_avg_loss'] = local_results_dict['loss'] / local_results_dict['total']
                     self.ctx.eval_metrics[f'{split}_acc'] = local_results_dict['correct'] / local_results_dict['total']

        # 이제 결과 처리가 끝났으므로, on_fit_end 훅 (메모리 정리 포함)을 실행합니다.
