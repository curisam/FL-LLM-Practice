...
class Trainer(BaseTrainer):
    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        # ... (on_fit_start, _run_epoch, on_fit_end 훅 실행까지는 동일) ...
        ...
        if hasattr(self, 'accelerator') and self.accelerator.num_processes > 1:
            ...
            if mode in [MODE.TEST, MODE.VAL]:
                # 평가 모드에서는 기존처럼 eval_metrics 사용
                metrics_to_sync = self.ctx.eval_metrics
            elif mode == MODE.TRAIN:
                # 학습 모드에서는 Monitor로부터 최종 학습 결과를 가져와서 
                # eval_metrics와 유사한 형태로 만듦
                # get_regular_log_res()는 {'total': ..., 'loss': ..., 'correct': ...} 형태를 반환
                train_results = self.ctx.monitor.get_regular_log_res()
                # 키 이름을 'train_total', 'train_loss' 등으로 바꿔줌
                if train_results.get('total', 0) > 0:
                    metrics_to_sync = {
                        'train_total': train_results.get('total', 0),
                        'train_loss': train_results.get('loss_total', 0.0),
                        # 정확도는 total로 나눠서 계산
                        'train_acc': train_results.get('correct', 0) / train_results.get('total')
                    }
                
            # 2. 결과 gather (이제 metrics_to_sync는 train 모드에서도 올바른 값을 가짐)
            gathered_metrics = self.accelerator.gather_for_metrics(metrics_to_sync)

            # 3. 메인 프로세스에서 합산
            if self.accelerator.is_main_process:
                aggregated_metrics = {}
                split = self.ctx.cur_split # 'train', 'val', 'test' 중 하나

                # ... (합산 로직은 거의 동일하지만, train_acc도 처리하도록 개선) ...
                ...
                # 4. 합산된 결과를 다시 적절한 위치에 저장
                if mode in [MODE.TEST, MODE.VAL]:
                    self.ctx.eval_metrics = aggregated_metrics
                else: # MODE.TRAIN
                    # 학습 결과는 monitor에 다시 업데이트 해주거나,
                    # 나중에 Client가 반환할 ctx의 메인 변수들을 직접 덮어쓰기
                    # 여기서는 train()의 반환값을 위해 eval_metrics에 임시 저장하는 것이 가장 간단할 수 있음
                    self.ctx.eval_metrics = aggregated_metrics 
                    # 또는 Monitor를 직접 업데이트
                    # self.ctx.monitor.update_regular_log_res(...)
            else:
                # 5. 다른 프로세스의 결과 비우기
                self.ctx.eval_metrics = {}
                if mode == MODE.TRAIN:
                    self.ctx.monitor.reset_regular_log_res()

        # train()의 반환값은 num_samples, model_para, results 임.
        # 이 results가 어디서 오는지 확인하는 것이 중요.
        # 만약 여기서 수정한 eval_metrics가 results가 된다면 성공.
