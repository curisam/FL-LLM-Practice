...
class Trainer(BaseTrainer):
    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        ...
        if using_accelerate:
            ...
            # 최종 전역 메트릭 계산 & 저장. reduce 후 (모든 rank가 같은 eval_metrics 보유)
            ...
            # ✅ (선택) 모든 프로세스의 로컬 dict를 rank0에 모으기
            try:
                gathered = gather_object(per_rank_entry)  # 모든 프로세스에서 호출
                if self.accelerator.is_main_process:
                    # 각 항목은 {'rank': i, '<split>_total': ..., ...}
                    self.ctx.per_proc_local_results = gathered  #0번 rank에서만 모은다.

                    """
                    [
                    {'train_total': 37, 'train_loss': 22.94, 'train_avg_loss': 0.620, 'train_acc': 0.649, 'rank': 0},
                    {'train_total': 36, 'train_loss': 21.60, 'train_avg_loss': 0.600, 'train_acc': 0.611, 'rank': 1},
                    {'train_total': 36, 'train_loss': 22.32, 'train_avg_loss': 0.620, 'train_acc': 0.667, 'rank': 2},
                    {'train_total': 37, 'train_loss': 20.68, 'train_avg_loss': 0.559, 'train_acc': 0.622, 'rank': 3}
                    ]
