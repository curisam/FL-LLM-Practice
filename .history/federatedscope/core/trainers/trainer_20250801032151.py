...
class Trainer(BaseTrainer):
    ...
    def train(self, target_data_split_name="train", hooks_set=None):
        """
        Run local training and aggregate results in a distributed environment.
        """
        hooks_set = hooks_set or self.hooks_in_train
        self.ctx.check_split(target_data_split_name)

        # 1. _run_routine을 호출하여 각 프로세스에서 로컬 학습을 수행합니다.
        #    이 함수가 끝나면 self.ctx에 `loss_total_train` 등의 로컬 결과가 누적됩니다.
        num_samples = self._run_routine(MODE.TRAIN, hooks_set,
                                        target_data_split_name)

        # 2. Monitor를 사용해 각 프로세스의 로컬 누적 결과를 깔끔한 딕셔너리로 변환합니다.
        #    이 작업은 메모리 정리 훅(`on_fit_end`)이 실행되기 전에 수행되어야 합니다.
        #    `monitor.eval`은 `train` 모드에서도 `ctx`의 누적값으로 결과 딕셔너리를 잘 만들어줍니다.
        local_results = self.ctx.monitor.eval(self.ctx)

        # 3. 분산 환경일 경우(accelerate 사용 시), 모든 프로세스의 로컬 결과를 통합합니다.
        if hasattr(self, 'accelerator') and self.accelerator is not None \
                and self.accelerator.num_processes > 1:
            
            # 모든 프로세스로부터 local_results 딕셔너리 객체를 수집합니다.
            # gather_object는 텐서가 아닌 일반 파이썬 객체를 모을 때 사용합니다.
            from accelerate.utils import gather_object
            gathered_results_list = gather_object(local_results)
            # 이제 gathered_results_list는 모든 프로세스의 로컬 결과 딕셔너리를 담은 리스트입니다.
            # 예: [{'train_total': 120, ...}, {'train_total': 120, ...}, ...]

            # 메인 프로세스(rank 0)에서만 최종 글로벌 결과를 계산합니다.
            if self.accelerator.is_main_process:
                final_results = {}
                # 집계할 키들을 정의합니다. (필요에 따라 추가/삭제)
                keys_to_aggregate = ['train_total', 'train_loss', 'train_correct']
                
                for key in keys_to_aggregate:
                    # 모든 프로세스의 결과를 합산합니다.
                    # .get(key, 0)을 사용하여 해당 키가 없는 경우에도 안전하게 처리합니다.
                    total_value = sum(res.get(key, 0) for res in gathered_results_list)
                    final_results[key] = total_value
                
                # 합산된 값을 기반으로 평균 지표를 계산합니다.
                if final_results.get('train_total', 0) > 0:
                    final_results['train_avg_loss'] = final_results.get('train_loss', 0) / final_results['train_total']
                    final_results['train_acc'] = final_results.get('train_correct', 0) / final_results['train_total']
                
                # 최종 통합 결과를 self.ctx.eval_metrics에 저장합니다.
                self.ctx.eval_metrics = final_results
            else:
                # 다른 프로세스들은 결과 딕셔너리를 비워서 중복 보고를 방지합니다.
                self.ctx.eval_metrics = {}
        else:
            # 단일 프로세스 환경에서는 로컬 결과를 그대로 최종 결과로 사용합니다.
            self.ctx.eval_metrics = local_results

        # 4. 최종적으로 모델 파라미터와 (통합된) 결과 딕셔너리를 반환합니다.
        return num_samples, self.get_model_para(), self.ctx.eval_metrics
