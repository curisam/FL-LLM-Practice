...
class Trainer(BaseTrainer):
    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        ...
        # [핵심] _hook_on_batch_end가 모든 필요한 값을 누적해 놓은 상태
        ...
        if num_samples > 0:
            ...
            # 만약 F1 score 등 다른 메트릭이 필요하다면, 여기서 monitor.eval()을 호출
            # 지금은 loss/acc만 필요하므로 직접 계산하는 것이 가장 안전
            if mode in [MODE.TEST, MODE.VAL]:
                # monitor.eval()을 호출하기 전에 ys_true를 텐서로 변환
                ...
                # local_results.update(self.ctx.monitor.eval(self.ctx)) # 필요시 이 라인 활성화



        # on_fit_end 훅 실행 "전에" 결과 처리
