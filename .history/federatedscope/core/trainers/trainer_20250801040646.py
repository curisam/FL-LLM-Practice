...
class Trainer(BaseTrainer):
    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        ...
        # === [최종 수정안] on_fit_end 훅 실행 전 ===
        
        split = self.ctx.cur_split
        local_results = {}
        
        if mode == MODE.TRAIN:
            # ctx에서 직접 결과를 계산 (loss_total과 correct가 0인 문제 인지)
            num_samples = self.ctx.get('num_samples', 0)
            # LLM Trainer는 ctx.loss_total 대신 다른 변수를 사용할 수 있음
            # 예: ctx.loss_batch_total_train (reward_choice_trainer 확인 필요)
            # 우선 가장 일반적인 이름으로 시도
            ...
            # eval/test는 eval_metrics에 결과가 잘 담겨서 온다고 가정
            # 키 이름에서 'test_'/'val_' 접두사 제거 (통일을 위해)
        ...
        if using_accelerate:
            # [수정] 딕셔너리를 텐서로 변환하여 gather
            # local_results가 비어있으면 [0, 0, 0] 텐서를 보냄
            if local_results:
                data_to_gather = torch.tensor(
                    [local_results.get('total', 0), 
                    local_results.get('loss', 0.0), 
                    local_results.get('correct', 0)],
                    device=self.accelerator.device
                )
            else:
                data_to_gather = torch.zeros(3, device=self.accelerator.device)

            # gather_for_metrics는 텐서를 잘 처리함
            gathered_tensors = self.accelerator.gather(data_to_gather)
            # gathered_tensors는 이제 (num_processes, 3) 모양의 텐서가 됨
            
            # 모든 프로세스가 동일한 최종 결과를 계산
            # 텐서 연산으로 모든 것을 합산
            ...
            if total_samples > 0:
                final_results = {
                    ...
                    f'{split}_correct': total_correct, # correct는 나중에 acc 계산에 사용
            ...
            if local_results:
                # 키 이름에 split 접두사 추가
                self.ctx.eval_metrics = {f'{split}_{k}': v for k, v in local_results.items()}
                if local_results.get('total', 0) > 0:
                    self.ctx.eval_metrics[f'{split}_avg_loss'] = local_results['loss'] / local_results['total']
                    self.ctx.eval_metrics[f'{split}_acc'] = local_results['correct'] / local_results['total']
            else:
