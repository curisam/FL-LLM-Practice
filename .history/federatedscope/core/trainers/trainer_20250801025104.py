...
class Trainer(BaseTrainer):
    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        ...
        if hasattr(self, 'accelerator') and self.accelerator.num_processes > 1:
            ...
            # [수정 1] train 모드일 때 metrics_to_sync를 직접 생성
            if mode == MODE.TRAIN:
            ...
            # eval/val 모드는 기존처럼 ctx.eval_metrics를 사용
            elif mode in [MODE.TEST, MODE.VAL]:
                metrics_to_sync = self.ctx.eval_metrics
            
            # 동기화할 내용이 없으면 여기서 종료
            if not metrics_to_sync:
                # 다른 프로세스와의 동기화를 위해 eval_metrics를 비워줌
                if mode in [MODE.TEST, MODE.VAL, MODE.TRAIN]:
            ...
            if self.accelerator.is_main_process:
                ...
                # [수정 2] train, val, test 모드 모두 결과를 eval_metrics에 덮어쓰기
                self.ctx.eval_metrics = aggregated_metrics
            else:
                self.ctx.eval_metrics = {}

        # [수정 3] 단일 프로세스 환경일 때 train 결과를 eval_metrics에 담아주는 로직 추가
        elif mode == MODE.TRAIN:
            # monitor.eval()은 train 결과를 계산해서 반환해줌
            self.ctx.eval_metrics = self.ctx.monitor.eval(self.ctx)
