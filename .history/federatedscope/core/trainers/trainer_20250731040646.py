...
class Trainer(BaseTrainer):
    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        ...
        # wrapper 진입 (lifecycle("routine")가 붙었으므로)

        #### ctx.track_mode(mode) → ctx.cur_mode를 TRAIN/TEST/…로 바꾸고 model.train()/eval() 호출

        #### ctx.track_split(dataset_name or mode) → ctx.cur_split를 "train"/"test"로 설정
        ...
        if mode in [MODE.TEST, MODE.VAL] and hasattr(self, 'accelerator') \
