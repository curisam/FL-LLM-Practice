...
class Trainer(BaseTrainer):
    ...
    def train(self, target_data_split_name="train", hooks_set=None):
        ...
        # 1. _run_routine으로 로컬 학습을 수행하고, ctx에 로컬 결과 누적
        num_samples = self._run_routine(MODE.TRAIN, hooks_set,
                                        target_data_split_name)

        # 2. monitor.eval()을 호출하여 ctx의 로컬 누적 결과를 딕셔너리로 변환
        #    이것은 메모리 정리 훅(`on_fit_end`)보다 먼저 실행되어야 합니다.
        local_results = self.ctx.monitor.eval(self.ctx)

        # 3. 분산 환경(accelerate) 여부 확인
        using_accelerate = (hasattr(self, 'accelerator') and 
                            self.accelerator is not None and
                            self.accelerator.num_processes > 1)

        if using_accelerate:
            # 3a. 모든 프로세스의 로컬 결과 딕셔너리를 수집
            from accelerate.utils import gather_object
            gathered_results_list = gather_object(local_results)

            # 3b. 메인 프로세스(rank 0)에서만 최종 글로벌 결과 계산
            if self.accelerator.is_main_process:
                final_results = {}
                # 집계할 숫자 키들을 정의 (필요에 따라 추가/삭제)
                # monitor.eval()은 'train_correct' 대신 'train_acc'를 직접 계산할 수 있음
                # 안전하게 .get()으로 접근
                total_samples = sum(res.get('train_total', 0) for res in gathered_results_list)
                
                if total_samples > 0:
                    total_loss = sum(res.get('train_loss', 0) for res in gathered_results_list)
                    # 정확도는 가중 평균으로 계산
                    weighted_acc_sum = sum(res.get('train_acc', 0) * res.get('train_total', 0) for res in gathered_results_list)
                    
                    final_results = {
                        'train_total': total_samples,
                        'train_loss': total_loss,
                        'train_avg_loss': total_loss / total_samples,
                        'train_acc': weighted_acc_sum / total_samples
                    }
                
                # 최종 통합 결과를 self.ctx.eval_metrics에 저장
                self.ctx.eval_metrics = final_results
            else:
                # 다른 프로세스는 결과 딕셔너리를 비워서 중복 보고 방지
                self.ctx.eval_metrics = {}
        else:
            # 단일 프로세스 환경에서는 로컬 결과를 그대로 최종 결과로 사용
            self.ctx.eval_metrics = local_results

        # 4. 최종적으로 모델 파라미터와 (통합되었거나 로컬인) 결과 딕셔너리를 반환
        return num_samples, self.get_model_para(), self.ctx.eval_metrics
