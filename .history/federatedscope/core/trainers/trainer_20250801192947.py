...
class Trainer(BaseTrainer):
    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        ...
        # on_fit_end 훅이 실행되기 "전에" 결과 처리
        split = self.ctx.cur_split
        local_results = {}
        
        # [핵심] 모드에 따라 결과 생성 방식을 다르게 함
        if mode == MODE.TRAIN:
            # Train 모드: ctx의 누적값으로 직접 결과 딕셔너리 생성
            # 이렇게 하면 monitor.eval()을 우회하여 KeyError/AttributeError를 피함
            num_samples = self.ctx.get('num_samples', 0)
            if num_samples > 0:
                loss_total = self.ctx.get('loss_batch_total', 0.0) # LLMTrainer는 이 변수 사용
                correct = self.ctx.get('correct', 0) # RewardChoiceTrainer가 추가
                local_results = {
                    f'{split}_total': num_samples,
                    f'{split}_loss': loss_total,
                    f'{split}_avg_loss': loss_total / num_samples if num_samples > 0 else 0,
                    f'{split}_acc': correct / num_samples if num_samples > 0 else 0,
                }
        elif mode in [MODE.TEST, MODE.VAL]:
            # Eval/Test 모드: 기존 방식대로 monitor.eval() 호출
            # 단, 호출 전에 ys_true가 list이면 tensor로 변환하여 RuntimeError 방지
            if hasattr(self.ctx, 'ys_true') and isinstance(self.ctx.ys_true, list) and self.ctx.ys_true:
                self.ctx.ys_true = torch.concatenate(self.ctx.ys_true)
            if hasattr(self.ctx, 'ys_pred') and isinstance(self.ctx.ys_pred, list) and self.ctx.ys_pred:
                self.ctx.ys_pred = torch.concatenate(self.ctx.ys_pred)
                
            local_results = self.ctx.monitor.eval(self.ctx)

        # on_fit_end 훅 실행 (메모리 정리 포함)
        for hook in hooks_set["on_fit_end"]:
            hook(self.ctx)

        # 분산 환경 통합 로직 (이 부분은 모든 모드에 공통으로 적용)
        using_accelerate = (hasattr(self, 'accelerator') and 
                            self.accelerator is not None and
                            self.accelerator.num_processes > 1)

        if using_accelerate:
            from accelerate.utils import gather_object
            gathered_results_list = gather_object(local_results)

            final_results = {}
            if gathered_results_list and isinstance(gathered_results_list[0], dict):
                total_samples = sum(res.get(f'{split}_total', 0) for res in gathered_results_list)
                if total_samples > 0:
                    total_loss = sum(res.get(f'{split}_loss', 0) for res in gathered_results_list)
                    weighted_acc_sum = sum(res.get(f'{split}_acc', 0) * res.get(f'{split}_total', 0) for res in gathered_results_list)
                    final_results = {
    ...
    #     # 1. 모드에 따라 로컬 결과 딕셔너리 생성
    #     #    _hook_on_batch_end가 ctx 변수들을 채워준 상태입니다.
    #     num_samples = self.ctx.get('num_samples', 0)
    #     if num_samples > 0:
    #         if mode == MODE.TRAIN:
    #             loss_total = self.ctx.get('loss_total', 0.0)
    #             correct = self.ctx.get('correct', 0)
    #             local_results = {
    #                 f'{split}_total': num_samples,
    #                 f'{split}_loss': loss_total,
    ...
    #             }
    #         elif mode in [MODE.TEST, MODE.VAL]:
    #             # 평가 모드에서는 ys_true/ys_pred 리스트를 텐서로 변환
    #             if hasattr(self.ctx, 'ys_true') and isinstance(self.ctx.ys_true, list) and self.ctx.ys_true:
    #                 self.ctx.ys_true = torch.concatenate(self.ctx.ys_true)
    #             if hasattr(self.ctx, 'ys_pred') and isinstance(self.ctx.ys_pred, list) and self.ctx.ys_pred:
    #                 self.ctx.ys_pred = torch.concatenate(self.ctx.ys_pred)
                
    #             # 이제 monitor.eval()은 안전하게 텐서를 처리
    #             local_results = self.ctx.monitor.eval(self.ctx)
    ...
    #     # AttributeError 방지를 위해 항상 eval_metrics를 빈 딕셔너리로 초기화
    #     self.ctx.eval_metrics = {}

    #     for hook in hooks_set["on_fit_start"]:
    #         hook(self.ctx)

    #     # _run_epoch()은 ctx에 num_samples, loss_total, correct 등을 누적
    #     self._run_epoch(hooks_set)
    ...
    #     # 1. 모드에 따라 로컬 결과 딕셔너리 생성
    #     num_samples = self.ctx.get(f'num_samples_{split}', self.ctx.get('num_samples', 0))
    #     if num_samples > 0:
    #         if mode == MODE.TRAIN:
    #             # train 모드에서는 ctx에서 직접 누적된 값을 읽어옴
    #             loss_total = self.ctx.get(f'loss_total_{split}', self.ctx.get('loss_batch_total', 0.0))
    #             correct = self.ctx.get(f'correct_{split}', self.ctx.get('correct', 0))
    #             local_results = {
    #                 f'{split}_total': num_samples,
    #                 f'{split}_loss': loss_total,
    #                 f'{split}_avg_loss': loss_total / num_samples,
    ...
    #             }
    #         elif mode in [MODE.TEST, MODE.VAL]:
    #             # --- [핵심 수정] monitor.eval() 호출 전에 텐서 변환 ---
    #             # 평가 모드에서는 ys_true, ys_prob 등이 list of tensors 형태로 쌓여있음
    #             # MetricCalculator가 처리할 수 있도록 하나의 큰 텐서로 합쳐줌
    #             if hasattr(self.ctx, 'ys_true') and isinstance(self.ctx.ys_true, list) and self.ctx.ys_true:
    #                 self.ctx.ys_true = torch.concatenate(self.ctx.ys_true)
    #             if hasattr(self.ctx, 'ys_prob') and isinstance(self.ctx.ys_prob, list) and self.ctx.ys_prob:
    #                 self.ctx.ys_prob = torch.concatenate(self.ctx.ys_prob, dim=0)
    #             if hasattr(self.ctx, 'ys_pred') and isinstance(self.ctx.ys_pred, list) and self.ctx.ys_pred:
    #                 self.ctx.ys_pred = torch.concatenate(self.ctx.ys_pred)
                
    #             # 이제 안전하게 monitor.eval() 호출 가능
    #             local_results = self.ctx.monitor.eval(self.ctx)
