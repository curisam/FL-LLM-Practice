    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        ...
        if using_accelerate:
            ...
            if total_all > 0:
                ...
                # (옵션) generic 키도 채움
                final_results['total']    = final_results[f'{split}_total']
                final_results['loss']     = final_results[f'{split}_loss']
                final_results['avg_loss'] = final_results[f'{split}_avg_loss']
                final_results['acc']      = final_results[f'{split}_acc']
            ...
            if num_samples > 0:
                local_results['total']    = local_results.get(f'{split}_total', 0)
                local_results['loss']     = local_results.get(f'{split}_loss', 0.0)
                local_results['avg_loss'] = local_results.get(f'{split}_avg_loss', 0.0)
                local_results['acc']      = local_results.get(f'{split}_acc', 0.0)
            self.ctx.eval_metrics = local_results
            ret_num_samples = num_samples
