...
# federatedscope/core/sampler.py (발췌)
class ClusterUniformSampler(UniformSampler):
    def __init__(self, client_num, config=None, **_):
        super().__init__(client_num)
        adp = getattr(config, 'llm').adapter
        self.clusters   = [set(g) for g in adp.clusters]               # 1-based
        self.boundaries = list(adp.boundaries)                          # [end0, end1, ...]
        self.s_per      = [int(x) for x in adp.sample_num_per_adapter]  # [s0, s1, ...]
        self._allowed   = set()
        self._cur_s     = None
        self._cur_aidx  = None          # 로그용 메타

    def _adapter_idx(self, round_idx: int) -> int:
        from bisect import bisect_right
        aidx = bisect_right(self.boundaries, int(round_idx))
        if aidx >= len(self.s_per):     # 경계 초과 방어(평가 라운드 등)
            aidx = len(self.s_per) - 1
        return aidx
    ...
    def sample(self, size: int):
        # cluster 모드에서는 호출 인자 size 대신 현재 라운드의 s_i 사용
        size = int(self._cur_s) if self._cur_s is not None else int(size)
        idle = np.nonzero(self.client_state)[0].tolist()   # Sampler 표준: 1-based도 가능
        ...
        logger.info(f"[ClusterSampler] round={getattr(self,'_last_round','?')} "
                    f"aidx={self._cur_aidx} | s={size} picked={picked} "
