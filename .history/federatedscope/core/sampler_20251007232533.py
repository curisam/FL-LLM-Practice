...
class ClusterUniformSampler(Sampler):
    """
    boundaries(누적 물리라운드) 기반으로 현재 라운드의 어댑터를 찾고,
    그 어댑터의 클러스터 내부에서만 균등추출.
    cluster 크기<=5면 '전원참여'가 되도록 sample_num_per_adapter가 설정되어 있음.
    """
    def __init__(self, client_num, clusters, boundaries, sample_num_per_adapter):
        super().__init__(client_num)
        self.clusters = [set(g) for g in clusters]   # 내부 0-based
    ...
    def _adapter_idx(self, round_idx: int) -> int:
        # boundaries = [P0, P0+P1, ...]; r < B[i] 인 첫 i
    ...
    def sample(self, size: int):
        # size는 무시하고 _cur_s를 사용
        size = int(self._cur_s) if self._cur_s is not None else int(size)
        idle = np.nonzero(self.client_state)[0]  # 내부 0-based
        candidates = [i for i in idle if i in self._allowed]
        if len(candidates) < size:
            raise ValueError(f"[ClusterSampler] 후보 {len(candidates)} < 요청 {size} ; 구성을 확인하세요.")
