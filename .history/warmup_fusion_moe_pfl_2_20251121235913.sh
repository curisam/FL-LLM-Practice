...
CFG="fedbiscuit_script/tldr/finetune_warmup_fusion_moe_config_u3.yaml"
...
run_one () {
  local ema="$1"
  local outdir_path="exp/tldr/choice_qwen/pfl/warmup_fusionmoe_u3_1.0_${ema}"

  # 동적으로 경로 업데이트
  local load_grouping_weights_path="exp/gfl/warmup_fusionmoe_u3_1.0_${ema}/grouping_weights_round160_beta${ema}.json"
  local pretrained_model_path="checkpoints_1.0/final_tldr_choice_qwen_warmup_fusionmoe_u3_${ema}_round_180.ckpt"
  local eval_outdir="exp/tldr/choice_qwen/pfl/warmup_fusionmoe_u3_1.0_${ema}/raw"
  local save_outdir="exp/tldr/choice_qwen/pfl/warmup_fusionmoe_u3_1.0_${ema}/"
